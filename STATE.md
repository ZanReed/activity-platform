...
**Phase 1 — Stage 9a (schema additions for the checkpoint/feedback system).** Stages 1–8 complete (playground week through serialize layer). New feature design complete via architecture review: per-section checkpoints, student-facing feedback at submission, revision cycles with attempt tracking, confidence ratings, and pedagogical metadata (activityType, standards). Now landing the schema-level changes that everything else depends on; DB migration (9b) and Tiptap section_break (9c) follow. Editor-to-Supabase wiring (Stage 10) is the next gate after that. Runtime architecture decisions captured in `packages/renderer/RUNTIME.md`.
...
**Last updated:** Architecture review for checkpoint/feedback/revision system complete. RUNTIME.md created at `packages/renderer/RUNTIME.md` capturing all runtime architecture decisions (state shape, render diffing, data-attribute contract, build pipeline, error handling, testing strategy). Stages 7 (MathLive) and 8 (serialize layer with 18 round-trip tests) marked complete. Stage 9 split into 9a (schema), 9b (DB migration + ingest-submission), 9c (Tiptap section_break). New stages 10–16 laid out for editor-to-Supabase wiring, runtime work, editor UI for new fields, and dashboard. Pedagogical additions agreed: three submission modes (single / locked / free), revision modes (free / locked), per-blank hint + mistakeFeedback + solution, opt-in confidence rating, activityType enum, standards tagging at activity and problem level. Phase 2 pedagogical block types catalogued (worked example, faded worked example, learning objectives + success criteria, self-explanation). Next session resumes at Stage 9a schema additions — `document.ts` changes already drafted in the prior conversation; awaiting paste of `fill-in-blank.ts`, `problem.ts`, submissions schema file, factories, and existing tests to give precise diffs for those.
