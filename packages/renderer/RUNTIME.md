...
Bundle size (as of the 2026-07-10 graph-runtime-into-kit move): base runtime IIFE is **~21.2 KiB minified — over the 20 KiB soft target** (the numeric strategy and multiple-choice machinery are core question code that belongs in base, not behind `graphExt`; the 40 KiB hard ceiling is still far off); the graphs variant is **~23.8 KiB** (base + the ~2.6 KiB bridge; was ~27.8 before the plumbing moved into the lazy kit). Future graph runtime features grow the kit, not these numbers. Re-check with `pnpm bundle:renderer`, which prints both — treat the printed numbers as the truth, not this paragraph.
