...
**⚠ APPLY migration `0022_purge_section_checks.sql`** (`supabase db push`) — fixes a latent bug that would have stopped the **entire nightly retention purge**, not just part of it. `purge_soft_deleted` (0003) predates `section_checks` (0020) and deletes a purge-eligible activity's *versions* before the activity itself; `section_checks.activity_version_id` is ON DELETE RESTRICT, so the first check old enough to matter raises `23503`, and since the job is one plpgsql function that one exception rolls back assignments, activities, and account purges too — visible only in the cron log. **Not firing yet** (verified live: 44 checks exist, none on a soft-deleted activity), but soft-deleting the E2E activity `6a84c8cb` and waiting 30 days is all it would take. **Reproduced against the live function and confirmed fixed, both inside rolled-back transactions** — nothing committed; see `scripts/verify-0022.sql` section C, which keeps the reproduction. No redeploy needed (pg_cron calls it inside the DB). After applying, run `scripts/verify-0022.sql`.
