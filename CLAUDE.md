...
- **The anonymous student-facing functions must always be redeployed with `--no-verify-jwt`: `ingest-submission`, `get-feedback`, AND `get-activity`.** The first two are called from published pages with NO Authorization header; `get-activity`'s anonymous branch is the 3.2A pre-auth meta endpoint. The platform's default `verify_jwt` gate 401s those requests before the function runs. **`supabase/config.toml` is the authoritative record of every function's flag** (added 2026-08-06, policy P6 — a decision recorded only in an npm script is not recorded); the CLI reads it on deploy, so even a raw `supabase functions deploy` now inherits the right flag. Before it existed, a plain redeploy silently re-enabled JWT verification (this bit get-feedback on its first deploy — advisor/`list_edge_functions` caught `verify_jwt:true`). Still prefer `pnpm deploy:ingest` / `pnpm deploy:feedback` / `pnpm deploy:get-activity` (flag baked in as belt-and-braces; if script and config.toml ever disagree, config.toml is the intent and the script is the bug), and **verify the live flags with `list_edge_functions` after the next deploy** (the author action that closes A2); `pnpm deploy:train` walks a full multi-part deploy in the safe order. **`check-activity` (S4 grading) also stays `verify_jwt:true`** and is deliberately NOT in the trio — a check is always made by a signed-in student, so the platform gate is a free first line of defense; deploy it with `pnpm deploy:check` and never add `--no-verify-jwt`. (**`publish-activity` was DELETED at S9 Drop 1** — publish is a direct `publish_activity` RPC from `usePublish`; there is no publish deploy step. There is no `upload-image` function anymore — the editor uploads images straight to the `activity-images` Storage bucket under 0019's RLS INSERT policy.)
