...
**🚨 pg_cron IS NOT INSTALLED — SO NO SCHEDULED JOB IN THIS PROJECT HAS EVER RUN, INCLUDING `purge_soft_deleted`.** Found 2026-08-04 while scheduling S7's maintenance job: `pg_extension` has no `pg_cron` row and there is no `cron` schema, and nothing else in the repo schedules anything (no workflow `schedule:`, no scheduled function). **This is the 0022–0025 pattern one level up:** that arc fixed the purge *function* and proved it works in rolled-back transactions, but nothing ever checked that it was *scheduled*. It never was. Practical impact today is ~zero (0 students, 0 `section_checks`, 3 users, a handful of activities, nothing soft-deleted long enough to matter), which is exactly why it stayed invisible. **The gap is a documentation one right now:** [retention-policy.md](docs/compliance/retention-policy.md) names a "scheduled purge job" as the enforcement mechanism for four data categories, and no such schedule exists. **⚠ An attempt to enable it on 2026-08-04 did NOT take.** Re-checked immediately after, connected to database `postgres` as `postgres` (where Supabase installs pg_cron): no `pg_extension` row, `pg_available_extensions.installed_version` NULL, no `cron` schema, no `cron.schedule` function. Since `cron.schedule` does not exist, the two scheduling statements cannot have run — they would have failed with *schema "cron" does not exist*. **Check the SQL editor output for errors rather than assuming the statements took**; that is the same silent-failure shape as the finding itself. Most reliable path: run `create extension if not exists pg_cron;` in the SQL editor (or dashboard → Database → Extensions → pg_cron → enable), confirm with `select extname from pg_extension where extname='pg_cron';`, and only then register BOTH jobs —
...
— then confirm with `select jobname, schedule, active from cron.job;`. Until then, treat every retention window in the compliance pack as designed-but-unenforced, and say so if the question ever comes up.
...
3. **▶ Enable pg_cron and schedule BOTH jobs** — see the 🚨 item above. This is now the oldest outstanding item in the project, not an S7 chore.
4. **▶ `cp .env.supabase.example .env.supabase`**, add the service-role key, then **`pnpm backfill:census -- --dry-run`** and, if it reads right, **`pnpm backfill:census`**. Current state: **134 versions, 0 censused** — no content read has hit the new function yet, so nothing is counted. Also **3 published current versions have no current-rev cache row** and **3 stale-rev orphans** (`1-5dbcb651` vs current `1-f8328527`) are waiting for the GC. Rerunnable by design; it is also the repair tool.
5. **Then** paste `scripts/verify-analytics-e2e.js` into the browser console while signed in (the deployed-function half of the proof), and open `/activity/:id/analytics` to see the panel. Re-running `scripts/verify-0026.sql` in the SQL editor is optional — its sections were executed live and green today (above), but the committed script is the reproducible form and now carries the `slug` fix.

**Otherwise EMPTY — verified against the live project, not assumed** (2026-08-04, post-0026: `list_migrations` + `list_edge_functions` + a pg_proc/pg_extension pass): **migrations applied through 0026**; `ingest-submission` v42 / `get-feedback` v4 / **`get-activity` v8** all `verify_jwt:false`; `check-activity` v5 + `publish-activity` v122 both `verify_jwt:true`; kit manifest `graph-kit-R5LUPQJS.js` committed. **Not empty in one respect: no scheduled jobs exist at all — see the 🚨 pg_cron item above.** Deploy narratives archived in [HISTORY.md](docs/HISTORY.md).
...
| Scheduled jobs (pg_cron) | 🚨 **NONE — pg_cron not installed; `purge_soft_deleted` has never run.** See Pending author actions |
