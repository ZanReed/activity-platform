...
3. **R2 teardown station** — [s9-cutover.md](docs/design/s9-cutover.md) §8 verbatim. **Both pre-checks RAN 2026-08-14; read these before the station:**
   ...
   - ✅ **Secrets + origins DONE 2026-08-15 (author, via dashboard → Edge Functions → Secrets):** the 7 zero-reader secrets deleted (5 R2_* + the 0029-era relics `IP_HASH_SALT`/`SUBMISSION_ENDPOINT`); `ALLOWED_ORIGINS` = localhost + pages.dev. **All four live probes GREEN:** pages.dev preflight 204 + ACAO echo; dead R2 origin refused (cors.ts answers unknown origins with its first allowed entry — a mismatch the browser blocks, not a missing header); localhost echo; get-activity boots post-restart (its own 400 validation JSON, not a platform error).
   - ✅ **BUCKET DELETED 2026-08-15 — the R2 teardown station is CLOSED end to end.** Probe: the old public base now answers **401**, not 200 (Cloudflare's response for an unbound public host; not a 404, so read it as "no longer serving" rather than as proof-of-delete — the author's action is the primary evidence). Archive retained at `r2-final-backup-20260815/` (283 objects, 68M, untracked). **R2 is now absent from the product end to end:** no code path, no script, no secret, no bucket.
