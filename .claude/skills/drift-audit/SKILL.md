...
- `MATHLIVE_VERSION` — pinned in `packages/graph-kit/src/mathlive-setup.ts` (⚠ this bullet said `packages/app/src/lib/mathlive-setup.ts` until the 2026-08-24 §0 pass — a ⚰ path that has never existed, do not look for it) and **self-guarding**: `packages/app/vite.config.ts`'s `activity:mathlive-fonts` plugin fails the BUILD if the pin drifts from the installed package. Note the guard exists; do not re-verify by hand.
