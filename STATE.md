...
3. **Deploy the comma-key kit change** (`dd5466f`): `packages/graph-kit` changed, so the bundle re-hashes. Per the standing kit-deploy rule below — run `pnpm build:graph-kit` (upload FIRST), confirm the `Uploaded:` lines, then `supabase functions deploy publish-activity --project-ref dtqutpdplefmufrrakxs`, then re-publish calculator activities. Folds into action #2's re-publish.
...
| Calculator tool (Phase 2.7) | ✅ Stages 1–4 + full graphing UX overhaul (floating panel, in-panel MathLive keyboard, splitter, hover-trace, sticky axes, calc results) — all browser-verified at `/dev/calculator` ([HISTORY 2026-07-06](docs/HISTORY.md)). Kit `graph-kit-ZMCQEGJA.js` uploaded + `publish-activity` redeployed 2026-07-06. Comma key added `dd5466f` (re-hashes the kit → needs re-deploy, pending action #3). Next: Stage 5 (graded interactions) |

Test counts at last session: schema 120 / graph-kit 71 / renderer 298 / app 257; `tsc -b` + app build + lint green. (Environment note: this machine is the 2026-07-04 recovery clone — old computer died; toolchain re-set-up verified by a full green run. Deploy creds for `pnpm build:graph-kit` live in a gitignored `.env.r2`, auto-loaded — see `.env.r2.example`.)
...
**Last updated:** 2026-07-06. Completed-session narratives are archived in [HISTORY.md](docs/HISTORY.md) (rolling log, newest first); durable reasoning in [DECISIONS.md](docs/DECISIONS.md). Suite at last session: schema 120 / graph-kit 71 / renderer 298 / app 257; CI green locally (typecheck/lint/test/build).
