...
- **Static published HTML is GONE — the S9 cutover deleted it (2026-08-14, Drops 1–4).** Students get the live-API viewer SPA (`@activity/viewer` + the `get-activity` read API + server-authoritative `check-activity` grading; answer keys never reach clients). `packages/renderer`, the publish/ingest/feedback Edge Functions, the anonymous submission wire (migration 0029), and the Phase 2.6 Submissions dashboard are all deleted; the R2 kit/font upload machinery is DELETED too (the D-13 teardown ran 2026-08-15 — scripts and `.env.r2` gone); only the bucket itself and its dashboard settings await author-side removal. Every mention of the renderer, the runtime, published-page HTML, or the ingest wire in the phase bodies below is **history**, kept for the reasoning — not a live surface.
...
| **2.6b** | AI grading assist | Model-drafted rubric grades a teacher confirms/edits/rejects — suggestions never grades ✅ **built + live-verified 2026-09-26** (ai-grading-assist.md, 0042, `packages/grading-worker`, queue pre-fill UI). ⚠ Model half PARKED — the local-inference pilot waits on the author's GPU box (months); D7 hand-grading proceeds on student volume alone. The subscriber service path (hosted API, quota-gated) is seamed in but gated on compliance + hosted-model validation (§7a) |
...
**Architectural delta**: Everything from scratch. Postgres schema with append-only versioning, RLS, signup trigger with allowlist, atomic publish RPC, ingest submission RPC. Edge Functions for publish and submission. TypeScript packages for schema (Zod), renderer (pure JSON-to-HTML), and the React app (Vite + Tiptap). Static HTML on publish, hosted on Cloudflare R2 (Supabase Storage and Edge Functions both rewrite HTML responses to `text/plain` on free tier as anti-abuse, forcing the host off `*.supabase.co` — see Cross-cutting concerns / Hosting platform). *(as built then; all retired at S9 Drop 1–4, 2026-08-14)*
...
**Common question types beyond fill-in-blank**: multiple choice (single and multi-select) ✅ **SHIPPED 2026-07-10**; matching pairs + ordering / sequencing ✅ **SHIPPED 2026-07-10**; number-line (1-D) + data-plot (dot/histogram/box stats charts) ✅ **SHIPPED 2026-07-12**. These are basic question types in nearly every subject — and historically far more common than fill-in-blank in non-STEM classrooms — but they don't fit `BlankResponse` because the answer isn't a string. Each new type followed the parallel-map pattern established by graphs: a new `choices` / `matches` / `orderings` / `numberLineResponses` / `dataPlotResponses` map on `SubmissionResponses`, a new block type in the discriminated union, and a new NodeView. **As-built:** the wire bump landed at v5 for MC (graphs took v3/v4 first), v6 for matching/ordering, v7 for number_line, v8 for data_plot. A selection-shaped question gets its own runtime machinery (`runtime/mcs.ts` / `matches.ts` / `orderings.ts`, scored by set/sequence equality), NOT a strategy in the `evaluateAnswer` dispatch (that dispatch is for typed-blank answer formats — list/numeric); the number_line and data_plot builds reuse the graph block→kit→pure-scorer template, riding the lazy graph kit behind their own bridge seams (`numberLineExt` / `dataPlotExt`).
...
**Architectural delta**: ~~Image upload to object storage (Phase 2 — likely R2 since it's already integrated for published HTML, with size limits and a cleanup policy).~~ **Resolved 2026-07-31: Supabase Storage bucket `activity-images` (0019).** Aggregate-stats query/view. Email verification flow in Supabase Auth. ~~Markdown parser package (likely a thin port of the existing bulk-importer.js).~~ **Shipped early in Phase 1** as `packages/app/src/lib/markdownToTiptap.ts` — markdown-it plus a small custom layer for `{{blanks}}` / `{checkpoint}` / `$math$`, not the bulk-importer port originally imagined. A new Tiptap mark (definition) joins the bold/italic/code mark set, carrying an inline definition string. Renderer emits `<span class="definition" data-definition="..." tabindex="0" role="button">` with full keyboard accessibility (Tab to reach, Enter/Space to open, Esc to dismiss; floating-ui for positioning, already a transitive dependency via the drag handle). Runtime gains a definition-popover handler — a new interactive element class in the data-attribute contract. Attribute names (`data-definition`, `data-glossary-key`) are designed now so the Phase 4 glossary layer is additive, not a rename.

For MC / matching / ordering: each gets its own optional parallel map on `SubmissionResponses` at a wire bump (MC shipped this at v5; ingest accepts all prior versions and migrates on write). The Edge Function's response validation accepts the new shape; the runtime grows a per-category module (MC shipped as `runtime/mcs.ts` — see the as-built correction above; matching/ordering follow that shape, not the `evaluateAnswer` dispatch). The renderer emits the same `data-block-category="question"` contract used by fill-in-blank, plus type-specific data attributes for choices and correct keys. Subscript / superscript marks add two new entries to the mark schema, two new toolbar buttons, two new Tiptap mark extensions, and pass-through serialize / renderer support.

**Decisions deferred to the start of this phase**:
- ~~Image hosting limits and lifecycle: per-teacher quota? Auto-delete after N days of non-use? Bucket — same R2 bucket as published HTML (with separate prefix), or a dedicated bucket? Probably dedicated, for cleaner lifecycle policies and quotas.~~ **Resolved 2026-07-31: Supabase Storage bucket `activity-images` (0019)** (quota/lifecycle still open).
...
> ⚠ **Correction (2026-08-22):** mechanism sketch predates S9 — there is no runtime or data-attribute contract; scoring is the server's (`check-activity`).
...
> **⚰️ RETIRED at S9 Drop 3 (2026-08-14, ruling OV-5).** The Submissions dashboard WAS the grading UI and died with the anonymous wire; the student-feedback half (get-feedback) never worked live (P9 audit — bodiless 200s from birth). What survives: the short_answer/essay blocks + rubric authoring (editor/schema/viewer) and the empty `grades` table. The ~~**parked**~~ **teacher-grading slice — ✅ SHIPPED 2026-08-16 (`docs/design/teacher-grading.md`, migration 0034)** — owns the successor, bound to `section_checks`; trigger unchanged — the first real teacher. The section below is design history for that slice to mine, not shipped behavior.
...
✅ **Substantially shipped and live (2026-06/07).** Calculator Stages 1–4 (scientific → graphing → regression → expression list, plus a large UX overhaul), and the graded graph block with point / function (incl. rays and segments) / inequality / region interactions plus a static display mode — all on the one shared kit, all live-verified 2026-07-10. Both design docs (`docs/design/calculator-tool.md`, `docs/design/interactive-graph-block.md`) carry as-built delta notes — **the mechanisms sketched in this section's "Architectural delta" were superseded** (shared R2 kit + a graphs runtime build variant behind the `graphExt` seam, not a per-publish `graph-widget.js`; kit scorers, not `evaluateAnswer` strategies). Remaining from the "further advanced" tail: stats beyond regression, and whatever graded families teachers ask for next. The original framing follows.
...
> ⚠ **Correction (2026-08-22):** student identity (S1) and the admission model (`docs/design/admission-model.md`, migration 0033, shipped 2026-08-15 — join codes, `redeem_join_code`) already shipped; Phase 3 is Classroom/LMS integration only. The anonymous-with-typed-name sketch below is pre-S9 history.
...
**Cross-repo obligation at phase start (not a decision — a signal owed; RETARGETED 2026-09-25)**: D32 was superseded by **D36** — the curriculum side already re-banded (`deriv.*`/`function.*` → Y12, `limit.*` collapsed) on AS-standards evidence alone. When Phase 5 publishes year-by-year content, post a row on the platform-owned Notion boundary page naming the **D36 re-band VERIFICATION** (check the ruling against the published content); the verification itself is curriculum-side. Artifact: `decision-log-additions.md` in ZanReed/curriculum, entries D32 (supersession line) + D36. The boundary page carries the matching standing tickler.
...
- **Not a grading tool for high-stakes assessment.** ✅ *Addressed early — server-authoritative grading shipped (S4, 2026-08-01; `check-activity`); answer keys no longer reach clients.* ~~See "security ceiling" above.~~ Formative practice, exit tickets, homework — yes. Final exams — no, until Phase 5+ if ever.
