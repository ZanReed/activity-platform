...
**📤 UNPUSHED: `eb2494a` — shell slimming slice 1 (app-only).** Stubs the two Supabase sub-clients; entry chunk 177.6 → 156.4 KiB gz, cap 185 → 172. **No migration, no deploy, no bundle regeneration** — but a push IS a Cloudflare Pages deploy, so it ships the raw-fetch image upload to the live editor. Nothing here calls a new table or RPC, so OV-7's ordering rule does not bite. Local proof: full suite green, 16/16 budgets, integration lane 9/9 against a real stack, dev parity confirmed in a browser. (Two sw offline-reopen e2e rows fail locally and reproduce identically on a clean `main` — environmental; see the Suite line.)
...
## Current focus — shell slimming slice 1 SHIPPED (app-only, unpushed)

**[shell-slim-supabase.md](docs/design/shell-slim-supabase.md), plan + eng review + the AS BUILT section.** `@supabase/realtime-js` (+ phoenix) and `@supabase/storage-js` (+ iceberg-js) are aliased to inert stubs in `packages/app/src/lib/supabase-stubs/`; `lib/uploadImage.ts` — the one storage caller — makes its two calls as raw `fetch`. **Entry chunk 177.6 → 156.4 KiB gz, cap tightened 185 → 172 in the same commit** (ruling R6: a budget that has stopped protecting is a fossil). **App-only: no migration, no deploy, no bundle regeneration.** Cloudflare Pages auto-deploys on the author's push, and nothing here calls a new table or RPC, so OV-7's ordering rule does not bite.

**Four things a future session needs from this slice:**
1. **`supabase.channel()` and `supabase.storage.from()` now THROW** — with messages naming `vite.config.ts`, the design doc, and the un-stub path. The alias is runtime-only, so **TypeScript still autocompletes both as working**; the throw is the first signal. Un-stub at the realtime-push backlog arc, deliberately.
...
## Prior focus — the check-prune slice (0035) is BUILT; the rollup deliberately WAITS
...
**Suite (tool-verified 2026-08-18, after the shell-slim slice):** schema 340 / graph-kit 384 / **viewer 1139** / **app 1092** unit, plus **50 script tests**, the print e2e lane, and the editor/student/sw/perf/**a11y** e2e lanes (the four CI lanes run **73** rows — a11y is 11 with the Responses-tab axe row; the local-only integration lane is **9**). Typecheck + lint clean (0 errors); all **16 perf budgets pass**. ⚠ **Two sw offline-reopen rows fail LOCALLY on this machine and reproduce identically on a clean `main`** (stash-and-rerun proven 2026-08-18) — environmental, not a regression; CI's last run was 73/0. ⚠ **This line does NOT pin the bundle sizes any more** (drift audit 2026-08-17: three different entry-chunk numbers were in circulation across two STATE rows). **`node scripts/check-perf-budget.mjs` prints the real numbers and its caps live in `scripts/perf-budgets.mjs` with their reasoning** — read those, never a doc's copy. Same for test counts: `pnpm test`.
...
**Last updated:** 2026-08-18 (shell slimming slice 1 built and committed locally — app-only, UNPUSHED; the rollup's first v2 run at 03:30 UTC is still the thing to check).

**The slice's own headline: two of the plan's claims were wrong, and the tests written to honor them are what found it.** (1) "Vitest parity — dissolved by inspection" was half-right in a load-bearing way — vitest reads `vite.config.ts` but externalizes `node_modules`, so the alias never applied and V1–V3 would have passed while exercising the real library. The anti-vacuity row written *because* this review had already retracted one fictional safety net went red on its first run. (2) Severe finding 1's premise — an apikey-less upload 401s — is false against a real stack; the header is right to send, but the gate is the session token plus 0019's policy, and the liveness probe now fires *that*. **The pattern to keep: when a review's own claims are the input, the build's first job is to make each one falsifiable.** Also worth carrying: every absence marker was verified PRESENT in a pre-slice build before being trusted, which is what turned OV-8's warning from advice into three rejected marker candidates.
