...
- **Vocabulary definitions / glossary (design captured Stage 9c-era; inline = Phase 2, shared store = Phase 4):** teacher selects text → "Define" → types an inline definition. An inline `DefinitionMark` (not a node) renders `<span class="definition" data-definition…>` (additive to the frozen data-attribute contract) with a runtime popover; Phase 4 adds the centralized `ActivityMeta.glossary` map keyed by stable keys (`glossaryKey` attr name reserved now to avoid a later rename). Design in [docs/design/vocabulary-definitions.md](docs/design/vocabulary-definitions.md). Predates the current toolbar / popover-host / print-CSS work — needs a short design refresh (toolbar affordance, print behavior, mobile/touch interaction) before coding; none of those touch the schema or data-attribute contract.
