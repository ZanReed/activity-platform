...
2. **`publish-activity` RE-deploy + activity re-publish — reference panel (sidecar + two live-test fixes).** The renderer bundle emits the reference panel (bottom-bar toolbar + print box, gated by `meta.print.printReferencePanel`) and inlines a ~1 KiB sidecar (`runtime/reference-panel.ts`) — top-edge drag-resize + a `ResizeObserver` for scroll-clearance — only when an activity has a panel. Author deployed `publish-activity` once on 2026-06-18, but two bug fixes landed AFTER that deploy: body now scrolls (cap moved to the body, was overflowing) and the resize handle moved to the panel's top edge. So: **re-run `supabase functions deploy publish-activity`** (to pick up the fixed bundle) **and re-publish the activity** (published pages are static HTML on R2 — they don't change until republished). Safe to batch; panel-less activities render byte-identical. No DB/migration; no `ingest-submission` change.

Cleared 2026-06-18: **`publish-activity` redeployed — order-independent blank groups verified live on a published page** (grouped scoring confirmed: any-order accepted, consume-once rejects duplicate answers; the `{{~}}` markdown import round-tripped through publish). Cleared 2026-06-18: **legacy Supabase Storage `activities` bucket deleted by author** (R2 verified end-to-end; the app's `.from('activities')` calls are the DB table, not the bucket — confirmed nothing referenced it; no env vars needed removing). Cleared 2026-06-17: **migration `0008_soft_delete_activity.sql` deployed — deleting activities now works** (soft-delete RLS bug; writeup in HISTORY 2026-06-17, reasoning in DECISIONS → "Activity deletion"). Cleared 2026-06-16: **cross-origin `<img>` loads from R2 confirmed working** by the author; **the three real-mouse GUI passes confirmed working** (drag-between-cells, image resize handles, nested rich-text mini-editor). Also: `publish-activity` redeployed with the **submission-payload fix** (runtime now POSTs snake_case `activity_id`/`display_name` matching `ingest-submission`) and the `[TEST] E2E Coverage` activity re-published — live submit verified end to end (see Current focus). Cleared 2026-06-13 (verified via Supabase API): `publish-activity` was at **v34**, carrying the full variable-block-sizing bundle (width fill + image fixed-height crop + cell `--cell-min-height`). Earlier (2026-06-12): migration 0007 applied; `upload-image` deployed; `ingest-submission` still has `verify_jwt: false`.

## Current focus

**▶ Phase 2 — Reference panel COMPLETE (Drops A + B + C shipped & committed).** Optional teacher-authored scaffold content (formula charts, vocab lists, conversion tables…). **A** (`76c40f7`): serialize round-trips `referencePanel` as editor state — closed a latent drop-bug. **B** (`0ba81bf`; renderer bundle regenerated): renders the panel — an on-screen collapsed bottom-bar toolbar (`<details>` + flex `column-reverse`, opens upward, zero runtime JS) + a print box at the worksheet top, gated by the new `meta.print.printReferencePanel` toggle. **C** (`9bd8b4d`): the authoring surface — an isolated, constrained `ReferencePanelEditor` (text + math + lists + columns + image; no sections, no blank-authoring UI) behind a collapsible "Reference panel" disclosure in the editor route, wired into autosave (fingerprint on the panel's Tiptap JSON, not the serialized panel). Reasoning in [DECISIONS](docs/DECISIONS.md) → "Reference panel". The **scoring** runtime stays untouched (the panel renders outside `.activity-section`); a separate ~1 KiB sidecar (`runtime/reference-panel.ts`, inlined only when a panel exists) adds author-requested **drag-resize** (top-edge handle) + **scroll-clearance** (a `ResizeObserver` pads the page by the panel's live height so the fixed bottom panel never permanently hides content), revising the earlier zero-JS call (see DECISIONS). All green: schema 89 / renderer 280 / app 241, typecheck clean; screen toolbar + authoring editor verified live earlier, and the resize/scroll-clearance mechanics verified deterministically via the published-HTML harness (a live-drag screenshot just needs a foregrounded preview window). **Pending author actions:** (1) redeploy `publish-activity` (above) so published panels render; (2) a manual end-to-end check — author a panel → publish → confirm the bottom-bar toolbar + print box (the editor route needs auth, which Claude can't drive). **Follow-up chip queued:** remove the redundant Underline extension (StarterKit v3 bundles it → benign "duplicate extension names" warning in the main editor). Earlier: Phase 1 loop complete + verified end-to-end (order-independent blank groups shipped 2026-06-18 — see Recently completed). Other Phase 2 "decide at phase start" items still open: image-hosting quota, MC/matching/ordering UI, block-type priority, `skills` editing UI, custom R2 domain.
