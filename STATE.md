...
3. **Calculator (Phase 2.7): upload the graph-kit bundle(s) to R2 + redeploy `publish-activity`.** Run `pnpm build:graph-kit` **with R2 creds in env** — it emits THREE `.js` files (code-split) and uploads all of them to `shared/`: the entry `graph-kit-JAOMFO5I.js` (267 KB gz, now incl. Stage 3 regression), the JSXGraph chunk `graph-kit-chunk-CTM4ZPXU.js` (240 KB gz, fetched only in graphing mode), and a tiny shared chunk (`graph-kit-chunk-PBY3QZDK.js`). Then redeploy `publish-activity` (it reads `_shared/graph-kit-manifest.ts` → the entry filename → `calculatorKitUrl`). **Required** for any published calculator to work; the summon button 404s gracefully until done. Use the creds route, not manual dashboard upload — there are 3 files and they must all land in `shared/`. (Hashes change whenever `packages/graph-kit` changes — the committed manifest, regenerated 2026-07-04 with Stage 3, is the source of truth.) MathLive fonts come from jsDelivr (no upload). See [supabase/functions/README.md](supabase/functions/README.md) → "Graphing kit asset on R2".
...
**▶ Phase 2.7 — Graphing track. Stages 1–2 SHIPPED; Stage 3 (data table + regression) CODE-COMPLETE (2026-07-04).** The **`@activity/graph-kit`** package is the kit, consumed two ways from one impl (published pages lazy-`import()` it; the editor preview imports it). Stage 1 (scientific, [HISTORY 2026-06-21](docs/HISTORY.md)) + Stage 2 (graphing, JSXGraph lazy-split into its own on-demand chunk, [HISTORY 2026-06-23](docs/HISTORY.md)) both browser-verified. **Stage 3 (`2bc3f68` + `9df8738`, [HISTORY 2026-07-04](docs/HISTORY.md)):** the Texas Algebra I regression flow — a `Data` toggle in graphing mode opens a two-column view (data table + model dropdown + aria-live results beside the board); students type (x, y) pairs (auto-append rows), pick linear/quadratic/exponential, and get the equation + r² with the fit curve dashed over the scatter. TI-84/Desmos conventions pinned in tests (exponential `y=a·bˣ`, r² on the log fit; quadratic labeled R²); `y ≤ 0` gets a clear message. New restriction flag `allowedRegressionModels` (default all, empty = off; additive, no schemaVersion bump) with per-model editor checkboxes + live preview. Engine (`regression.ts`) + table (`data-table.ts`) are standalone kit pieces for the future graded-regression block. ⚠️ **Interactive browser pass pending** (another session held the dev-server port): exercise `/dev/calculator` — (1,4) (2,7) (3,12) (4,21) → Exponential should read y = 2.3094 · 1.7356ˣ. **To go live: author action #3** — `pnpm build:graph-kit` with R2 creds (hashes changed again with Stage 3) → redeploy `publish-activity`. **Next: Stage 4** — multi-expression list (N rows, sliders, points, color coding — the Desmos-defining surface). Decisions in [docs/design/calculator-tool.md](docs/design/calculator-tool.md). Suite: schema 117 / graph-kit 52 / renderer 298 / app 257.
...
| Calculator tool (Phase 2.7) | ✅ Stage 1 (scientific) + ✅ Stage 2 (graphing, JSXGraph lazy-split) browser-verified; ✅ Stage 3 (data table + regression, `2bc3f68`+`9df8738`) code-complete, unit/build-verified, ⚠️ interactive browser pass pending. ⏳ Pending author action #3 (build:graph-kit upload + `publish-activity` redeploy) to reach published pages. Next: Stage 4 (multi-expression list) |
...
**Last updated:** 2026-07-04. Completed-session narratives are archived in [HISTORY.md](docs/HISTORY.md) (rolling log, newest first); durable reasoning in [DECISIONS.md](docs/DECISIONS.md). Suite at last session: schema 117 / graph-kit 52 / renderer 298 / app 257; CI green locally (typecheck/lint/test/build).
