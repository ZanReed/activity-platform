...
**Next major feature (author-chosen): structural authored columns** — a row/column container block so columns become part of the *content* and render consistently online + in worksheet print + inside a foldable panel, retiring the per-mode `Columns` print settings. See Nearest next steps → Active next goal.
...
**▶ Active next goal — structural authored columns (a row/column container block).** Replace the per-mode CSS `Columns` print settings with a *content* primitive: the teacher places blocks side by side, so columns render identically everywhere — on screen, in worksheet print, and inside a foldable panel. This is the better unifying design (the print-only `column-count` flow model is fundamentally incompatible with the foldable's fixed-region packing; making columns structural sidesteps that). **Scope:**
   - **Schema** (`@activity/schema`): a new container block (e.g. `ColumnsBlock` / `RowBlock` with `columns: Block[][]`, an optional per-column width ratio). Additive, optional — keep the no-`schemaVersion`-bump discipline if possible.
   - **Renderer** (`@activity/renderer`): emit the container as a CSS grid/flex row of column cells; each column renders its child blocks via the existing block path. Must look right both on screen and under `@media print`.
   - **Editor** (Tiptap): a `columns` node + NodeView (insert 2/3 columns, drag/type blocks into each), and `serialize.ts` round-trip (Tiptap ↔ `ColumnsBlock`).
   - **Foldable**: the measure step must treat a column container as **one measurable unit** — `measure.ts` already walks `.activity-section > *`, so a top-level container is measured/packed whole by `paginate`; verify it doesn't split mid-container and that its inner width resolves against the panel width.
   - **Retire (or soft-deprecate) the per-mode `Columns` print controls** once structural columns land — they become redundant. The foldable already hides its Columns control; the worksheet one can follow.
