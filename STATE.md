...
**⏭ NOW: the admission slice is COMPLETE and LIVE (T1–T7). Two commits are unpushed; one step needs a human.** The S9 arc closed 2026-08-15; the work since is the admission re-ruling ([admission-model.md](docs/design/admission-model.md) §5b R1–R11, eng + design CLEAR).

**⚠ AUTHOR STEP 1 — push `2f31d26` + `7e96797`.** No ordering constraint: 0033 is already live, and the pre-auth fork is UI-only — it calls no new table or RPC, so OV-7's migration-before-push rule has nothing to hold here. Pushing auto-deploys the landing to Pages.
...
## Current focus — NEXT: the teacher-grading slice (unparked 2026-08-15)

**The components-as-data arc (S0–S9) and the admission slice are both DONE.** The next arc is the **teacher-grading slice**, unparked because the admission work expired the condition it was parked on: "no teachers using the system right now" is no longer structurally true once a stranger can sign in and attest. See Backlog for the pickup notes and the two traps (`get-feedback` is not a reference implementation; the Phase-2.6 dashboard was deleted at S9 Drop 3). **Re-derive against shipped reality before building (P10)** — the slice's context predates both S9 and 0033.
...
**Suite (tool-verified 2026-08-15 late):** schema 340 / graph-kit 384 / viewer 1126 / **app 1057** unit, plus **38 budget-script tests**, the print e2e lane, and the editor/student/sw/perf/**a11y** e2e lanes (the four CI lanes run 71 rows; a11y is now 10 with the pre-auth landing's axe row). Typecheck + lint clean (0 errors); all 12 perf budgets pass (entry **176.3**/185 KiB gz). Treat the tools' printed numbers as truth, not this line.
...
| Database migrations 0001–0033 | ✅ **All applied + verified live** (list_migrations, 2026-08-15 late). 0031+0032 were REPRODUCIBILITY migrations, no-ops on live; **0033 is the admission slice** (`pending` role, `redeem_join_code`, `claim_teacher` + caps). Re-run `verify-0013-0014.sql` + `verify-0017.sql` after any auth/RLS/grant migration |
...
**Last updated:** 2026-08-15 (late, second session) — **The admission side quest is CLOSED, and it closed by finding something the slice had missed.** Reading the shipped signed-out Home against the approved design board — rather than against T3's task line, which was satisfied — showed that **R5-DR's ruled PRIMARY surface was never built**: the pre-auth fork (code door + teacher door before Google) existed only as the post-auth safety net, and `Home.tsx` carried a comment asserting "the pre-auth fork routes them", a claim about a surface that did not exist (P11, and the reason the gap survived a whole slice). Author ruled build-it-now; shipped as **T7** (`2f31d26`): `SignedOutLanding`, intent riding the redirect URL exactly as the join code already does, the DR-6 definitive-negative check moved BEFORE the OAuth round trip, and `PendingOnboarding` gaining `initialView` so the teacher door lands on attestation. **The P5 flip came with it** — the identity e2e row pinning the replaced "Sign in to continue" heading was flipped to the landing that replaced it, not deleted.

**What was verified, and the line between the two halves:** the PRE-AUTH half is now proven against the LIVE endpoint in a real browser (bad code → the DR-6 warning + its `role=status` announcement; `7NE9M2` → "Algebra 1" in the title slot; meta 200/763 ms, 404/2007 ms for the negative — the 404 is the endpoint working, not a defect). The POST-AUTH half — OAuth round trip, the trigger admitting a stranger as `pending`, `retryRole` after promotion — **cannot be proven by any AI session, because it requires signing in**; it is author step 2 above, with the expected neutral-gate blip documented so it is not misread as a bug. Suite 1057 (9 new rows), a11y 10/10 with a new axe row for the landing, identity 15/15, 12/12 budgets at **176.3/185 KiB** (+0.6, priced per the gate-9 ledger rule), typecheck + lint clean.

**Also closed: the counsel packet and the §7 split.** [counsel-review-packet.md](docs/compliance/counsel-review-packet.md) states the draft-2 → draft-3 delta plainly, opens with the live position (teacher self-serve already open, author-accepted, 3 teachers / 0 students / nothing advertised), and asks nine questions that name the platform's current position. Writing it caught real drift — `data-map.md` claimed migrations 0001–0027 while documenting 0033's columns. Under-13 (D7), the DPA template and the cap-lifting surface moved to TODOS with triggers; §7 now points instead of restating, because **an item described in two places drifts and this project has been bitten by that twice in one week**.

**One local-e2e note for the next session, since it will look alarming:** the full student lane went 5 red at load 4.44 (3.1m wall clock) — all five in `failure-matrix.e2e.ts`, the tab-lock/shared-device family, none of them touched by this work. Re-running the identity spec alone: 15/15 in 20.5s. That is the documented load artifact (TODOS: "local e2e timing results are not trustworthy under load"), not a regression. **Check `uptime` first, then isolate the spec you changed.**

*(Prior entry:)* **The admission slice T1–T6.** Self-serve admission built: unknown Google sign-ins admitted as a contained `pending` role (0033), promoted by two audited RPCs — `redeem_join_code` (code → student, promote+join in one call) and `claim_teacher` (attestation → teacher, capped 5 classes / 50 members). The reshape came from the eng review's outside voice killing the morning's email+password design (Supabase's built-in mailer cannot serve one classroom; the password path armed a pre-creation attack) and the author's counter-proposal replacing it — five findings deleted by one architecture change. **Three tests earned their keep before shipping:** `audit_action` is an ENUM so the promotion actions raised at RUNTIME inside the RPC (invisible to any lint); a containment row PASSED BY FAILING because the verify block runs as `postgres` and bypasses RLS; and the contract pin test came due exactly as its header warned, since 0033 replaces both `join_class` and the trigger. **P5 audits ran in the same commits:** verify-0027's two refusal assertions and the integration lane's outsider row were FLIPPED, not deleted — each now asserts the pending admission that replaced the refusal.
