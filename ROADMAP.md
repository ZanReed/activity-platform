...
| **2** | Polish the loop | Markdown import, image upload, better submission analytics, self-signup, common question types beyond fill-in-blank (MC, matching, ordering) |
...
**Integrity signals**: lightweight observational signals that help a teacher notice when a student likely didn't do their own work — primarily the copy-into-Gemini "no work done" pattern teachers report. Three signals, all captured by the runtime: per-blank **paste count** (answers pasted vs. typed), activity-level **visibility-loss events** (the student switched away from the tab/window and back), and activity-level **time-on-task** (how long the submission took overall). They cover orthogonal angles — paste count shows which specific answers were likely brought in from outside, visibility-loss shows whether the student left the page to get them, time-on-task shows whether the overall effort is plausible at all (a twenty-problem worksheet submitted in ninety seconds is its own signal). Time-on-task is deliberately coarse — one overall duration, not per-blank timing — because per-blank paste count already supplies the per-problem texture; a single number is lower-overhead and less noisy. All three surface to the teacher in the submission viewer as data beside each submission — never automated verdicts, never blocking submission. The teacher judges.

Paired with the signals is one deterrent: **clipboard poisoning** — copying text out of a `question`-category block appends a short notice to the clipboard, so a paste into a chatbot carries it. Scoped via `data-block-category` to question blocks only, so prose, callouts, and the reference panel stay freely copyable; framed as a trust-and-choice prompt, not a "you'll be caught" threat. A conscience nudge, not a wall — a student with a phone camera defeats any client-side measure, and that limit is accepted. This whole feature is explicitly observational, for the teacher's awareness, *not* enforcement: no lockdown, no blocked submission, no automated penalty — so it does not move the product into the time-limited, randomized, anti-cheat "quiz platform" lane that "What this product is NOT" rules out. Architecturally small: the runtime adds focus/paste/visibility listeners to elements it already wires, the counts ride in the submission payload as an additive optional structure (a per-blank paste map plus activity-level fields for visibility and time-on-task) at a Phase 2 `schemaVersion` bump with migrate-on-read, and the dashboard gains a column. No renderer data-attribute contract change, so nothing needs planting in Phase 1.
...
- How time-on-task is measured — wall-clock from page load to submit, from first interaction to submit, or active time (elapsed minus intervals the tab was hidden, reusing the visibility-loss instrumentation; this option requires visibility-loss to track durations, not just a count). Active time is the most honest; the others are cheaper. Decide at phase start.
...
**Architectural delta**: Three new block types — `audio_response`, `video_response`, `file_upload`. New parallel map on `SubmissionResponses`: `files: Record<uuid, { storagePath: string, mimeType: string, durationSec?: number, fileName: string }>` keyed by block.id. The actual media lives in a new Supabase Storage bucket separate from the public `activities` bucket — `student-submissions` is private, served via short-lived signed URLs to authenticated teachers only. The runtime gains a media-capture widget (MediaRecorder API for audio / video) and an upload widget (file input + drag-drop). On submit, the runtime uploads media to Storage via signed-upload-URL pattern (Edge Function issues the URL, runtime PUTs directly to Storage), then includes the resulting storage paths in the submission payload.

This is the privacy-sensitive phase. Student-generated media is a stronger privacy posture than typed text — voices and faces are identifying in ways that names aren't. Architectural commitments: media is never served publicly, signed URLs expire quickly (15 minutes), Storage bucket has strict RLS that matches the existing submissions-table RLS (only the assigning teacher / activity owner can issue signed URLs for a student's media), no anonymous access to media ever, audit log records every signed-URL issuance.
