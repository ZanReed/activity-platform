...
**⏳ IN FLIGHT (2026-08-07, session handoff): the local-first slice (`3cd4a7e`) was pushed and its CI run had not reported when the session ended.** Expected GREEN — the slice is client-code only (no bundles touched), verified locally: viewer 1079 / app 1047 env-less / student e2e 21/21 / scripts 33 / typecheck clean. If CI is red instead: the two new V7-matrix rows (failure-matrix.e2e.ts, steal + handback) are first-time-in-CI and timing-sensitive — read their traces before touching product code, and remember the masked-locally/red-on-CI family (check env assumptions first). While a green run is up, also append its perf numbers to the calibration ledger below (needs 5; has 2).

**Session ledger 2026-08-06→07 (all pushed unless noted):** eng review (28 rulings, 0 unresolved) → 41-item batch (9 commits) → CI un-redded (env-baked dist + math shutter + polled @page + 2× baseline regens) → B5 print heights → D2/D3 compliance rewrite + first POLICY_VERSION bump (author-approved) → local-first slice `3cd4a7e`. **Next in ruled order: (1) the B8 malformed-document slice** (typed `malformed_document` + walk integrity gate distinguishing authored-empty from structurally-broken + corpus red-green + client mapping to its own non-retry copy; grading-server bundle regen; `pnpm deploy:check` as pending author action — full ruling in findings-backlog.md → RULINGS → B8/D10 + the D29 amendment), **then (2) the S9-prep identity slice** (B12/B13/B14 rulings — role in SessionContext, student Home branch, join-by-code + /join/:code link, gate hardening migration FIRST, two-action remove dialog; BEFORE student_domain seeding), **then (3) S9 gets its own /plan-eng-review before build.** Unblocked opportunistic: the D23 privacy-hash TODO.
...
4. **D2/D3 pack rewrite: BUILT, awaiting your legal read (uncommitted working-tree diff, 2026-08-07).** Every changed sentence needs your eyes before it commits: the assertion's dropped under-13 clause, the departed-student ~400d disclosure, name→name+school-email, the Mechanics purge-job correction, and the three A11 gaps reconciled prose-side (activity-30d path disclosed; class purge + ip_hash scrub flagged unbuilt). `POLICY_VERSION` → `2026-08-07-draft-2`. **Standing gate recorded (D24): counsel/district reads the pack before the first real classroom** — the pack has never had legal review and every file says DRAFT.
