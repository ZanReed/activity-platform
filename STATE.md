...
**⚠ APPLY migration `0021_display_name_privacy.sql`** (`supabase db push`) — the server-side fix for the teacher-email privacy leak, RULED 2026-08-04 (reasoning in [DECISIONS.md](docs/DECISIONS.md) → "Teacher display_name is a name or nothing"). Three layers: signup trigger stores a name or NULL (never the email), `get_activity_public_meta` refuses to return an email-shaped value, and the author's row backfills to NULL (neutral "your teacher" attribution — deliberate, until the backlogged name control exists). **No Edge Function redeploy needed** — `get-activity` calls the RPC live, and the meta path is outside the `SANITIZER_REV` cache. After applying: run `scripts/verify-0021.sql`, then the regression re-runs (`verify-0013-0014.sql` A–D + `verify-0017.sql` — 0021 REPLACEd a function from each), then reload `/a/:id` signed out and confirm title + neutral attribution.
...
**Last updated:** 2026-08-04 — **display_name privacy fix ruled + built (migration 0021, pending apply above).** Trigger stops storing emails, anon meta RPC refuses to return them, backfill to NULL; name-appearance control backlogged. `scripts/verify-0021.sql` added; StudentViewer's client guard stays as the third layer (comment updated). Earlier same day: drift audit + STATE restructure (9 mechanical findings fixed, finished narratives to [HISTORY.md](docs/HISTORY.md)); suite verified green (counts above).
