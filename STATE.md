...
4. **`cp .env.supabase.example .env.supabase`**, fill in the service-role key, then **`pnpm backfill:census -- --dry-run`** and, if it reads right, **`pnpm backfill:census`**. Versions already sitting in the read cache were never re-read through the new code, so without this they carry no census and their checks show as `_unattributed`. Rerunnable by design — it is also the repair tool.
5. **Verify:** `scripts/verify-0026.sql` (sections 0→D; a green run prints notices and NO error — this one does not use the scary raise-exception-to-roll-back shape), then paste `scripts/verify-analytics-e2e.js` into the browser console while signed in.
...
| Migration 0026 (analytics census) | ⏳ Written + tested locally; **awaiting author apply** (see Pending author actions) |
