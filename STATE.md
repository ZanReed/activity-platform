...
1. **Push the batch commits, then watch CI.** The CI-red fix (env-baked dist + math-settled shutter + polled @page) should turn the perf/sw lanes and the rules gate green immediately; **if any of the four math-bearing print baselines still mismatches, dispatch the manual "generate print baselines" job** and commit its artifact — never regenerate locally (Linux-authoritative).
