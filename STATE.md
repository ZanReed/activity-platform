...
| Inline vocabulary definitions (mark + renderer span + runtime sidecar + editor authoring) | ✅ Code complete + browser-verified (`/playground`); ⏳ awaiting `publish-activity` redeploy to reach published pages |

Test counts at last session: schema 98 / renderer 288 / app 244; `tsc -b` + app build green. (Inline vocabulary definitions added: schema +9 marks.test, renderer +6 definitions sidecar + +2 render span, app +3 serialize round-trip.)
...
**Last updated:** 2026-06-19. Completed-session narratives are archived in [HISTORY.md](docs/HISTORY.md) (rolling log, newest first); durable reasoning in [DECISIONS.md](docs/DECISIONS.md). Suite at last session: schema 98 / renderer 288 / app 244; CI green.
