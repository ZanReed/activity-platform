...
**⏭ NOW: apply migration `0036_check_rollup.sql` (`supabase db push`), then `pnpm verify:auth --target live` — expect 147/0** (127 + verify-0036's 20). 0036 is the durable rollup + the self-healing timezone layer; it is **additive and non-destructive** — nothing it ships deletes a row, and the prune stays disarmed. No deploy, bundle, or push-ordering rule is implicated. Applied + verified locally 2026-08-16 (verify-0036 **20/20** first clean run; full local suite **145/2**, the 2 being the documented fresh-DB preconditions). Commit `f646b29`, unpushed.
...
**Baseline facts, tool-read live 2026-08-16 (never claim-read):** migrations applied **through 0035** (**0036 written + locally verified, pending author apply**) · exactly **TWO** Edge Functions — `get-activity` **v19** (`verify_jwt:false`, the only one) + `check-activity` **v15** (`true`) · `pnpm verify:auth --target live` = **127 passed / 0 failed across 11 scripts** (expect **147/0** after 0036) · both pg_cron jobs active · live rows (re-read 2026-08-16 after the 0035 apply): **3 teachers, 1 student, 0 pending, 0 checks, 0 grades, 1 class (`7NE9M2`, 1 active member), 8 activities** · **0 `analytics_job_runs` rows carry a `rolled_through`** — true until 0036 lands, after which the nightly job writes it and the gate becomes schedule-plus-dry-run (see the ⏭ block). ⚠ **"No real student data exists yet" is NO LONGER TRUE** — one real second account is enrolled. It is the author's own throwaway, so the compliance answers stay cheap to change, but the sentence that made them free has expired. **Re-verify flags with `list_edge_functions` after every deploy — read the `version` field, NOT the `entrypoint_path` suffix beside it** (that misread produced a false "verified live" stamp twice; `get-activity`'s path ends `…_11` while its version is 19).
...
| Database migrations 0001–0036 | ✅ 0001–0035 **applied + verified live (127/0 across 11 scripts)**. **0036 (the durable rollup + timezone layer) is WRITTEN, locally verified 20/20, and PENDING author apply** — expect live 147/0. 0031+0032 were REPRODUCIBILITY migrations; **0033 is the admission slice**; **0034 is checks-native grading**; **0035 is the disarmed check-prune + arming gate**; **0036 writes the watermark that gate reads**. Re-run `verify-0013-0014.sql` + `verify-0017.sql` after any auth/RLS/grant migration |
...
**Last updated:** 2026-08-16 (late; **0036 written, locally 20/20, pending apply** — 0035 is live, 127/0, CI-green at 31924079234) — **The months-parked pruning/rollup entry went from parked to fully built in one day, across two arcs and two eng reviews.** Morning: re-derivation (P10) → plan → review → **the outside voice overturned the ratified frame** ("build the rollup now" fell to "prune disarmed now, rollup at arming") → 0035 shipped and applied. Evening: the author pulled the arming arc forward, and Part II went plan → review → build → 0036.
