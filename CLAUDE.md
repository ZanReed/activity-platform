...
- **docs/skill-coverage-manifest.md** / **docs/skill-coverage.json** — generated by every import run; **neither exists until the first run after 0041 lands**. Author-refreshed, never CI-gated (the catalogue's `.md` files live outside this repo, so CI cannot regenerate them and no drift check could pass).
