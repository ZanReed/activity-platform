...
- [x] **T7 (P2) — FULLY DONE.** Shipped 2026-08-20: a universal `printExpectations` row (the written key never prints on a student worksheet) and an a11y row scanning the POST-CHECK worksheet, a state that lane had never scanned. **The half that was blocked is unblocked** — the viewer rendered no problem number for ANY block type, because the renderer's `isNumberedBlock` was the only implementation and died with `packages/renderer` at S9 Drop 4. [viewer-numbering.md](viewer-numbering.md) built that surface (V1–V9, 2026-08-20), so `numbering/prints` and the a11y numbering row now both exist.
