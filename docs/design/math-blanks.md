...
- [x] **MA-T4 (P1) — DONE (`e32ec04`, kit mount).** graph-kit `math-prompt-mount.ts` — read-only `MathfieldElement`, editable `\placeholder` prompts; hydrate restored ascii via `asciiToLatex`; `input`→`getPromptValue`→`latexToAscii`→`onValue`; hides the static KaTeX + inserts the field; `manual` VK policy. Returns a `MountedMathPrompts` handle (`setResult`/`reveal`/`destroy`). Owner-verified (MathLive-DOM, no jsdom); typecheck-clean against the real MathLive prompt API. **Deferred (MA-D5 follow-up):** wiring the handle's `setResult`/`reveal` into the runtime's check/reveal render so a checked gap turns green/locks in-field — grading is already correct via the mirror; this is the in-field visual sync.
