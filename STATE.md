...
2. **`publish-activity` redeploy — reference panel.** The renderer bundle emits the reference panel (on-screen bottom-bar toolbar + print box, gated by `meta.print.printReferencePanel`). All three drops have shipped, so a teacher can now author a panel — **redeploy is needed for published pages to render one.** Safe to batch (a panel-less activity still renders byte-identical). `pnpm bundle:renderer` regenerated + committed. No DB/migration; no `ingest-submission` change.
