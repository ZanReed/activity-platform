...
Bundle size (as of the 2026-07-10 numeric+MC session): base runtime IIFE is **~21.2 KiB minified — over the 20 KiB soft target** (the numeric strategy and multiple-choice machinery are core question code that belongs in base, not behind `graphExt`; the 40 KiB hard ceiling is still far off); the graphs variant is **~27.8 KiB** (only inlined on graph pages, held to the hard ceiling). The recovery lever when the trend matters: move the graph runtime plumbing INTO the lazy kit (already loaded on graph pages) so the base runtime stays lean. Re-check with `pnpm bundle:renderer`, which prints both — treat the printed numbers as the truth, not this paragraph.
