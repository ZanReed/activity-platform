...
- **No `@activity/schema` import.** The runtime defines its own minimal TypeScript interfaces mirroring the schema shapes. Deliberate duplication to keep the runtime tiny (size budget: `RUNTIME_SIZE_TARGET`/`RUNTIME_SIZE_CEILING` in `scripts/perf-budgets.mjs` — the one value-pinned home since S8's D5 unification; see Standing constraints for the reasoning). When schema shapes change, both sides update.
...
- **Manual feedback (Phase 2.6):** `short_answer` / `essay` each emit an empty `<div class="free-text-feedback" data-for-block="<uuid>" hidden>` after the textarea. A separate **feedback sidecar** (`runtime/feedback.ts`, inlined by `document.ts` only when the page has a `short_answer`/`essay` block — NOT in the base runtime) reads the `submission_id` the main runtime stored on submit, POSTs it to `config.feedbackEndpoint` (the get-feedback function), and — once the teacher has graded — fills each container with per-criterion scores + feedback and unhides it. `self_explanation` is ungraded and emits no container. Best-effort + silent on any failure; text is injected via `textContent` only.
- Print: each textarea prints as a bordered blank writing area (no answer-key variant — there is no key); the essay word counter and feedback panel are screen-only.
