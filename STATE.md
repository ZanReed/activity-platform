...
Stage 13 — Runtime: checkpoint scoring + feedback rendering. Stage 12 complete: the renderer emits the full data-attribute contract (blank wrappers + feedback slots, per-blank hint + mistakeFeedback, fill-in-blank block-level solution + confidence + skills, section-level checkpoint markup, container data-activity-type + expanded six-field config blob), and the runtime has the state-object architecture wired up (config.ts, refs.ts, state.ts, init.ts as new files; index.ts / blanks.ts / submission.ts refactored to consume refs + state rather than re-walking the DOM). Renderer bundle regenerated locally; still not redeployed to Edge Functions — the gap is now Stage 11 + Stage 12 combined.
...
| Renderer bundle for Edge Functions | ✅ Tracked in git as of 9b. Rebuild before any Edge Function deploy; bundle commits with the source it supports. ⚠ Regenerated by Stage 11 AND Stage 12 — pending redeploy. |
...
| Renderer emission contract — wrappers, hint/mistake, block-level, sections, container/config (Stage 12 steps 1–5) | ✅ Complete |
| Runtime state-object architecture — config/refs/state/init + 3-layer blanks split (Stage 12 step 6a+6b) | ✅ Complete |
| Runtime: checkpoint scoring + feedback rendering (Stage 13) | ⏳ Not started |
...
- **Renderer data-attribute contract (Stage 12 steps 1–5).** Locked-in choices: blank tokens are <input class="blank"> inside a <span class="blank-wrapper"> with a sibling <span class="js-blank-feedback" aria-live="polite" hidden> (void-element-can't-have-children fix); always-available hint ? button (not post-incorrect reveal); mistake feedback re-evaluated each check (no persistence); data-submission-mode deliberately NOT on the container in single mode — checkpoint markup is omitted entirely, not hidden; split-by-purpose contract (CSS hooks → data-* on .activity-container, JS-only config → six-field #activity-config blob); per-block (not per-blank) confidence fieldset, rendered after problem body. Full per-attribute contract lives in RUNTIME.md.
- **Three-layer blanks split (Stage 12 step 6b).** blanks.ts exposes scoreBlank(ref, typed) (pure: empty→null, else true/false), applyBlankFeedback(ref, correct) (DOM-only: toggles .correct / .incorrect classes), and checkBlank(ref) (convenience composition used by both blur handler and gatherResponses). Scoring is separated from DOM mutation today; Stage 13's render() builds on this. Architectural leak accepted: evaluateAnswer in strategies.ts still reads data-* off ref.input rather than taking parsed values — refactor candidate for Stage 13 if it becomes friction.
- **init.ts is the only DOM walker (Stage 12 step 6).** init(doc) returns {config, refs, state} | null; parseConfig returns null on missing/malformed config and the caller falls back to a no-op runtime. buildRefs walks sections → fill-in-blank blocks → blanks once and produces typed BlankRef / FillInBlankRef / SectionRef maps keyed by uuid. Downstream code consumes refs; nothing else calls querySelector against the body. Malformed per-element data attributes are warn-and-skip, not throw.
...
1. **Redeploy renderer bundle (Stage 11 + Stage 12 follow-up).** Stages 11 AND 12 both regenerated supabase/functions/_shared/renderer.bundle.js locally. Until it's redeployed, publish-activity is still running the pre-Stage-11 bundle — missing the schemaVersion: 1→2 fix AND the full Step 1–5 emission contract (blank wrappers, hints, mistake feedback, block-level solution / confidence / skills, section checkpoint markup, container data-activity-type, expanded six-field config blob). Do this before the next end-to-end test or before any further Edge Function work.
2. **Stage 13 — Runtime: checkpoint scoring + feedback rendering.** Builds on the state-object architecture now in place. Work: add per-blank-result + per-section-aggregate state to RuntimeState; introduce a proper render(state, refs) function (probably its own file) with the change-guard pattern; wire check-button click → score section blanks → write to state → render; hint button toggle (show/hide hint text + update aria-expanded); mistake-feedback dispatch (match typed value against data-mistake-feedback, show targeted message); solution reveal at check time (toggle .js-solution hidden); submission-mode enforcement (locked freezes inputs after check; free allows revision); localStorage answer persistence (tab refresh restores work). Tests added to existing JSDOM __tests__/ directory. ~2–3 sessions.
...
- **Multi-file delivery — verify count of tail outputs.** When delivering N files in one message and asking for tail -3 verifications on each, if the pasted output shows fewer than N blocks-of-3-lines, the missing file is almost certainly empty on disk (tail errors to stderr, which often doesn't paste alongside stdout). Don't assume "they probably saved" — explicitly ask the user to confirm. Discovered Stage 12 step 6a: config.ts was empty after paste; 5 tail commands produced only 4 visible blocks.
...
**Last updated:** Stage 12 complete — renderer emission contract + runtime state-object architecture. Steps 1–5 expanded the renderer output: blank tokens wrapped in <span class="blank-wrapper"> with sibling .js-blank-feedback slots; per-blank data-hint + .js-blank-hint button + .js-blank-hint-text + data-mistake-feedback JSON emission; fill-in-blank block-level data-solution + data-has-confidence-rating + data-skills plus inline .js-solution slot and per-block .js-confidence-rating fieldset; section-level data-is-checkpoint + .js-checkpoint-btn + .js-section-score (omitted entirely in single mode); container-level data-activity-type and the activity-config blob expanded from 3 to 6 fields. Step 6a added the runtime architecture as additive files (no behavior change, bundle bit-identical): config.ts (RuntimeConfig + defensive parseConfig), refs.ts (typed BlankRef / FillInBlankRef / SectionRef), state.ts (RuntimeState + createInitialState), init.ts (the single DOM walker producing {config, refs, state} | null), and JSDOM-backed __tests__/init.test.ts. Step 6b wired the architecture in: index.ts became the orchestrator (calls init, fails closed on null, hands InitResult to wireBlanks + submit); blanks.ts split into three layers (scoreBlank pure, applyBlankFeedback DOM-only, checkBlank composition); submission.ts consumes refs + state instead of re-walking the DOM; the local 3-field RuntimeConfig was replaced by the wider import from config.ts. New __tests__/blanks.test.ts covers the trim / score / feedback / composition rules. Behavior preserved for students (same blur-scores UX, same final-submit flow, same localStorage name persistence); runtime bundle grew by a few KiB minified but stays well under the 20 KiB target. Renderer bundle regenerated locally; still pending redeploy to Edge Functions — gap is now Stage 11 + Stage 12 combined. Next: Stage 13 — checkpoint scoring + feedback rendering on top of the state-object architecture now in place.
