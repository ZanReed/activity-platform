...
- **Runtime persistence schema bumps with shape changes.** `STORAGE_SCHEMA_VERSION` is currently 3 (1→2 when `BlankState` dropped `hintRevealed`; 2→3 with inline rich-text/math in hint/feedback/solution). If `BlankState`, `BlockState`, `SectionState`, or the blob shape changes incompatibly, bump it. Load returns null on mismatch → fresh state.
