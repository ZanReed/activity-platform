...
3. **Calculator (Phase 2.7): upload the graph-kit bundle(s) to R2 + redeploy `publish-activity`.** Run `pnpm build:graph-kit` **with R2 creds in env** — it emits THREE `.js` files (code-split) and uploads all of them to `shared/`: the entry `graph-kit-EQ5MV42W.js` (268.5 KB gz, incl. Stages 3–4), the JSXGraph chunk `graph-kit-chunk-X3OE7ZAZ.js` (240 KB gz, fetched only in graphing mode), and a tiny shared chunk (`graph-kit-chunk-PBY3QZDK.js`). Then redeploy `publish-activity` (it reads `_shared/graph-kit-manifest.ts` → the entry filename → `calculatorKitUrl`). **Required** for any published calculator to work; the summon button 404s gracefully until done. Use the creds route, not manual dashboard upload — there are 3 files and they must all land in `shared/`. (Hashes change whenever `packages/graph-kit` changes — the committed manifest, regenerated 2026-07-05 with Stage 4, is the source of truth.) MathLive fonts come from jsDelivr (no upload). **New-machine prereqs (2026-07-05):** in any terminal opened before setup, run `source ~/.zshrc` first (pnpm PATH); supabase CLI via `brew install supabase/tap/supabase` + `supabase login`; R2 creds recoverable from the Cloudflare dashboard. See [supabase/functions/README.md](supabase/functions/README.md) → "Graphing kit asset on R2".
