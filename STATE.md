...
**⏭ NOW: nothing is blocked. The teacher-grading slice is applied, pushed and CI-green; the admission slice is live.** **Nothing is unpushed** — `git ls-remote origin refs/heads/main` = local HEAD = `0006da0` (checked 2026-08-16, by the rule that `git status` cannot settle this). The two doc commits this line used to flag as pending did land. Everything below is a STANDING item with its own trigger, not a queue.
...
**Plan + rulings: [teacher-grading.md](docs/design/teacher-grading.md)** — eng review CLEAR (G1–G14), design review CLEAR (§2b/G8-DR, 4/10 → 9/10). T1–T6 built, 0034 applied, pushed, CI-green. **The surface is live and still correct-and-EMPTY**: **0 checks and 0 grades** means nothing has ever been graded on it, so the first real classroom is still its first real exercise. (As of 2026-08-16 there is now 1 enrolled student — but no student has ever *checked* anything, which is the input grading actually needs.)
