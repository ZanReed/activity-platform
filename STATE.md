...
**S8 IS BUILT (2026-08-05) — T1–T6 + T8 landed; T7 is an author decision, now measured.** The entry chunk went **2.86 MB → 592 KB raw (168.1 KiB gz)**: students no longer download Tiptap, the image cropper, or any authoring surface. `node scripts/check-perf-budget.mjs` enforces 12 deterministic budgets in CI (shell JS + CSS as separate rows, four shell-purity pins, a four-row content-matched chunk ledger, precache count + bytes); `node --test scripts/tests/*.test.mjs` covers the script itself with 23 cases including every vacuity branch. A new **perf Playwright lane** (11 specs) measures `performance.mark('student-interactive:*')` under 4× CPU + 5 Mbps throttling on the production build, and **the sw + student lanes now run in CI** (ruling D11). All three bundler ceilings moved into `scripts/perf-budgets.mjs` with their rationale, value-pinned against transcription error.

**Two things the author should know:**
1. **Timing targets are deliberately uncalibrated** (`TIMING_TARGET_MS` = null). The lane records every run and prints the numbers; local darwin values are pre-auth ~780 ms, worksheet ~840 ms. **Set them from the first green CI run** — a Linux runner is a different machine and baking a Mac number into the gate would be dishonest. Until then timing records but does not enforce (sizes always enforce).
2. ~~**T7 (KaTeX) is decision-ready.**~~ ✅ **RESOLVED 2026-08-05 — preload-on-math-detect built and measured.** The KaTeX fetch now starts the instant a served document is known to contain math, instead of waiting for a math component to mount. **The LaTeX-fallback window halved: ~737 ms → ~382 ms** (four confirming runs: 384/384/379/383), math typeset ~320 ms sooner, and the chunk stays lazy — a math-free document still downloads nothing, so ruling D16 is untouched and the shell is unchanged (168.4 KiB gz). Honest cost: worksheet-interactive is **~40 ms slower**, since the fetch now competes with the remaining render under throttling. Eager loading was NOT taken (+45% first load on math pages, and it would amend D16); the remaining ~380 ms residual is in TODOS.md with a trigger. Reasoning + before/after table in [DECISIONS.md](docs/DECISIONS.md) → "Preload math on detect".

*Prior status line:* **S8 perf-budget CI — eng review RULED 2026-08-05, build-ready** (14 decisions: 11 ruled + 3 long-term refinements; outside voice 12 findings, 5 ruled tensions; rulings, test plan, tasks T1–T8 in the gstack design doc → "S8 eng review" section; test-plan artifact `user-main-eng-review-test-plan-20260805-200915.md`). Headlines: entry split with StudentViewer+Home eager (entry chunk IS the student shell), ALL size budgets unified into a commented `scripts/perf-budgets.mjs` (incl. the three bundler ceilings, value-pinned), deterministic size gates + in-app `performance.mark('student-interactive')` (TTI-the-metric is dead) recorded per-PR with a derived 2× ceiling, lazy-mount smoke hardened against the RequireAuth vacuity trap, sw + student e2e lanes join CI, KaTeX resolution is a three-option author gate (lazy / eager / preload-on-math-detect). **Then S9 cutover** (S9 needs both parity gates green — the grading corpus gate and the print gate both already run in CI — plus the sw-offline fixme above). Task detail lives in the gstack design doc; nothing in S0–S7/S5.5 is outstanding. **S9 cutover cleanup checklist (2026-07-31 ruling, kept here so it isn't lost to HISTORY):** delete publish-activity's R2 upload path, the R2 secrets on the functions, `VITE_PUBLISHED_URL_BASE`, the R2 origin in `ALLOWED_ORIGINS`, then the bucket itself; the R2 kit machinery (`upload:graph-kit`, the manifest, `.env.r2`) and `build:fonts`/`build:mathlive-fonts` die with it; existing test-activity image `src`es on R2 keep resolving until the bucket dies, no rewrite needed. **⚠ AMENDED 2026-08-04 (S7): "the census folds into the S9 publish rewrite (R6)" is DEAD — S9 has no census work at all.** The census is derived from stored version snapshots by `get-activity`'s cache-fill path and never touches `publish-activity`, so the publish rewrite carries nothing analytics-related. Do not re-add a census write to the new publish path; it would be a second writer racing the read path on the same keys.
