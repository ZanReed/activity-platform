...
**▶ Active next goal — columns polish + variable block sizing.** The container exists; the remaining work is authoring affordances and the broader sizing vision it was built for:
   ...
   - **Image authoring — Drop 2 (NEXT, this session).** A new `upload-image` Supabase edge function mirroring `publish-activity`'s `aws4fetch` AwsClient R2 PUT (auth + ownership check; PUT to `uploads/{activityId}/{uuid}.ext`; return the public URL via `R2_PUBLIC_URL_BASE`). Wire the popover's Upload tab to it (file picker → POST → set src). Author deploys via `supabase functions deploy upload-image`. Not playground-testable (needs the deployed function).
   - **Remaining:** drag-between-cells, then the broader variable-block-sizing vision.
   ...
   - **Soft-deprecate the per-mode `Columns` print controls** now that structural columns render in every output (the foldable already hides its Columns control; the worksheet one can follow).
