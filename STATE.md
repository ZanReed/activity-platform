...
**⏭ NOW: apply migration `0035_check_prune_disarmed.sql` (`supabase db push`), then re-run `pnpm verify:auth --target live` — expect 127/0 (116 + verify-0035's 11).** 0035 is the DISARMED check-prune + the schema-encoded arming gate (view + nullable `rolled_through` + `prune_section_checks`, dry-run default, service-role only). Nothing calls it; no deploy, bundle, or push-ordering rule is implicated — it is safe to apply and push in either order. Applied + verified locally 2026-08-16 (11/11 first run; full local suite 125/2, the 2 being local-data preconditions — see the trap note below). Everything else below is a STANDING item with its own trigger.
...
**Baseline facts, tool-read live 2026-08-16 (never claim-read):** migrations applied **through 0034** · exactly **TWO** Edge Functions — `get-activity` **v19** (`verify_jwt:false`, the only one) + `check-activity` **v15** (`true`) · `pnpm verify:auth --target live` = **116 passed / 0 failed across 10 scripts** · both pg_cron jobs active · live rows (re-read 2026-08-16 after the admission proof): **3 teachers, 1 student, 0 pending, 0 checks, 0 grades, 1 class (`7NE9M2`, 1 active member), 8 activities**. ⚠ **"No real student data exists yet" is NO LONGER TRUE** — one real second account is enrolled. It is the author's own throwaway, so the compliance answers stay cheap to change, but the sentence that made them free has expired. **Re-verify flags with `list_edge_functions` after every deploy — read the `version` field, NOT the `entrypoint_path` suffix beside it** (that misread produced a false "verified live" stamp twice; `get-activity`'s path ends `…_11` while its version is 19).
...
| Database migrations 0001–0035 | ✅ 0001–0034 **applied + verified live** (116/0). **0035 (disarmed check-prune + arming gate) is WRITTEN and locally verified (11/11) — pending author apply**; after apply expect live 127/0. 0031+0032 were REPRODUCIBILITY migrations; **0033 is the admission slice**; **0034 is checks-native grading**. Re-run `verify-0013-0014.sql` + `verify-0017.sql` after any auth/RLS/grant migration |
