...
- **⚠ RETENTION IS NOT MECHANICALLY ACHIEVABLE YET — must close before the first real student account.** The submissions-clock question was RULED 2026-08-04 (the account waits for the work; 0023, reasoning in [DECISIONS.md](docs/DECISIONS.md)), but building it exposed that **no account can be purged at all**: `audit_log.actor_id` is NO ACTION and the signup trigger writes a `user.create` row for every account, so every account is permanently blocked by its own audit trail (verified live: 3 accounts, 3 blocked). 0023 makes that reported and non-fatal instead of a nightly crash; it deliberately does not resolve it, because the fix is another compliance ruling — **SET NULL the actor and keep the security event, delete the audit rows with the account, or make the account wait out audit_log's 2-year window**. Second gap in the same area: **nothing sets `users.deleted_at`**, so the account clock never starts; the marking step lands with the S4/S7 purge-job work. Both are named in [retention-policy.md](docs/compliance/retention-policy.md) → Mechanics, which counsel must read alongside the windows table.
