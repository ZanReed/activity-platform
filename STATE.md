...
**▶ Phase 1 loop complete + verified end-to-end; between phases (Phase 2 not yet kicked off).** Order-independent blank groups — the last in-flight Phase 1 item — shipped and verified live on a published page (2026-06-18): grouped scoring accepts any order, rejects duplicate answers (consume-once), and the `{{~}}` markdown import round-trips through publish. Reasoning in DECISIONS → "Order-independent blank groups"; data attribute in [RUNTIME.md](packages/renderer/RUNTIME.md) → "Blank token". No Phase 2 feature work has started — the "decide at phase start" decisions (image-hosting quota, MC/matching/ordering UI, block-type priority, `skills` editing UI, custom R2 domain) are all still open. **Next action:** the Phase 2 kickoff decision pass — surface those as a numbered list for the author's yes/no — or knock out the custom-R2-domain polish first.
