...
3. **Calculator (Phase 2.7): upload the graph-kit bundle(s) to R2 + redeploy `publish-activity`.** Run `pnpm build:graph-kit` **with R2 creds in env** — it now emits THREE `.js` files (Stage 2 code-split) and uploads all of them to `shared/`: the entry `graph-kit-SE7GLFDM.js` (264 KB gz), the JSXGraph chunk `graph-kit-chunk-7WK2PYJY.js` (240 KB gz, fetched only in graphing mode), and a tiny shared chunk. Then redeploy `publish-activity` (it reads `_shared/graph-kit-manifest.ts` → the entry filename → `calculatorKitUrl`). **Required** for any published calculator to work; the summon button 404s gracefully until done. Use the creds route, not manual dashboard upload — there are 3 files now, and they must all land in `shared/`. (Hashes change whenever `packages/graph-kit` changes — the manifest is the source of truth.) MathLive fonts come from jsDelivr (no upload). See [supabase/functions/README.md](supabase/functions/README.md) → "Graphing kit asset on R2".
...
**▶ Phase 2.7 — Graphing track. Stage 1 (scientific) CODE-COMPLETE + Stage 2 (graphing) SHIPPED.** The **`@activity/graph-kit`** package is the kit, consumed two ways from one impl (published pages lazy-`import()` it; the editor preview imports it). Stage 1 (5 slices, [HISTORY 2026-06-21](docs/HISTORY.md)): a teacher enables + restricts an ungraded on-screen scientific calculator (editor 🧮 disclosure + live preview); MathLive + math.js behind an engine-agnostic `evaluate()` seam. **Stage 2 — graphing (`5e513ac`):** a Mode selector (scientific/graphing) gives a graphing calculator that plots y=f(x) on a JSXGraph board (drag-pan, wheel-zoom, keyboard pan/zoom + aria-live a11y); a spike found **JSXGraph is ~240 KB gz** (2× the doc's stale estimate), so it's **code-split into its own chunk loaded only in graphing mode** — scientific stays ~264 KB gz, graphing pulls +240 KB gz on demand. `compileFunction()` added to the evaluate seam (y=f(x), deg/rad + gates, NaN-breaks); `board.ts` is the standalone board layer (dynamic-imported, so the graded block reuses it later without static JSXGraph). Browser-verified via `/dev/calculator` (plots x², sin(x); keyboard pan/zoom announces viewport; the `x` key builds x²). ⚠️ Editor sidebar still not auth-loaded (verified by proxy: the preview reuses the same `mountCalculator`, serialize round-trip + typecheck green). **To go live: author action #3** — `pnpm build:graph-kit` with R2 creds uploads the 3 split files → redeploy `publish-activity`. **Next: Stage 3** — data table + regression (linear/quad/exp, the Texas Algebra I requirement); the least-squares engine + table stay standalone for the future graded-regression block. Decisions in [docs/design/calculator-tool.md](docs/design/calculator-tool.md). Suite: schema 113 / graph-kit 32 / renderer 298 / app 250.
...
| Calculator tool (Phase 2.7) | ✅ Stage 1 (scientific) code-complete (5 slices) + ✅ Stage 2 (graphing, JSXGraph lazy-split, `5e513ac`); both browser-verified via `/dev/calculator`. ⏳ Pending author action #3 (build:graph-kit upload of the 3 split files + `publish-activity` redeploy) to reach published pages. Next: Stage 3 (data table + regression) |

Test counts at last session: schema 113 / graph-kit 32 / renderer 298 / app 250; `tsc -b` + app build green. (+ calculator Stage 2: graph-kit 32 incl. 6 compileFunction tests; board.ts browser-verified.)
...
**Last updated:** 2026-06-23. Completed-session narratives are archived in [HISTORY.md](docs/HISTORY.md) (rolling log, newest first); durable reasoning in [DECISIONS.md](docs/DECISIONS.md). Suite at last session: schema 113 / graph-kit 32 / renderer 298 / app 250; CI green.
