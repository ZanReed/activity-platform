...
2. **`publish-activity` redeploy — reference panel (+ resize/scroll-clearance sidecar).** The renderer bundle emits the reference panel (on-screen bottom-bar toolbar + print box, gated by `meta.print.printReferencePanel`) and now inlines a ~1 KiB sidecar (`runtime/reference-panel.ts`) — top-edge drag-resize + a `ResizeObserver` that keeps the page scrollable clear of the open panel — **only when an activity has a panel.** All authoring shipped, so a teacher can author a panel; **redeploy is needed for published pages to render one (with the new interactions).** Safe to batch (a panel-less activity renders byte-identical). `pnpm bundle:renderer` regenerated + committed. No DB/migration; no `ingest-submission` change.
