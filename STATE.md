...
- **⚠ For whoever writes the purge job: `section_checks` is NOT covered by the RESTRICT safety net.** Verified against the live schema 2026-08-04 while refreshing the compliance pack. `submissions.student_id` is ON DELETE RESTRICT (deleting a `users` row out of order fails loudly, which the retention doc leans on), but `section_checks.student_id` and `class_members.student_id` are ON DELETE **CASCADE** — deleting an account silently takes the student's whole check history with it, no error. That is the intended end state for a real purge, so nothing is unsafe today; the hazard is a mistaken `users` delete destroying check history with nothing to catch it. **The purge job should delete `section_checks` explicitly rather than leaning on the cascade**, so its reported row counts are the ones it meant to remove. Recorded in [retention-policy.md](docs/compliance/retention-policy.md) → Mechanics.
