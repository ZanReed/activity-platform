...
- **RUNTIME.md fill-in-blank contract drift.** The `### Fill-in-blank` section shows `data-block-category`/`data-block-type` on a single `<span class="blank-wrapper">`, but the renderer emits a two-level structure: a `<div class="block block-fill-in-blank">` block (now carrying block-category/type/id per the data-block-type reconciliation) containing `<span class="blank">` token elements. Three drifts: (1) element structure conflated; (2) class name — renderer emits `blank`, contract says `blank-wrapper`; (3) `data-blank-answers` format — renderer emits pipe-delimited (`correct|CORRECT|Correct`), contract shows a JSON array. Reconcile before Stage 12–13 builds fill-in-blank scoring against this contract.
