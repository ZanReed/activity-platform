...
**✅ Foldable × columns verification — done** (2026-06-13). Confirmed the journal-foldable engine handles a top-level `columns` container correctly: it's a single `.activity-section > *` child, so `measure.ts` treats it as one `FlowItem` (cells never flattened) and `paginate` never splits it across a fold; its `fr` tracks resolve against the fixed-width panel (narrow-screen collapse can't fire — it keys off the 1200px measure-iframe viewport). Locked in by a behavior-preserving extract (`extractFlowBlocks` lifted out of `measure.ts`) + `foldable-measure.test.ts` (4 tests) + a named `paginate` case, and proven in a real browser via a dev-only `/dev/foldable-columns` bench (2:1 container measured 286.7px / 143.3px = ratio 2, filling the 432px panel, single panel). Decision recorded in DECISIONS.md.

**▶ No active goal — next focus is open.** Variable-block-sizing, columns-polish, CI + housekeeping, and the foldable×columns verification are all complete. Pick the next from "Also queued" below.
...
- ~~**Foldable verification with a real columns block**~~ — ✅ done 2026-06-13. Confirmed (tests + real-browser bench) a top-level columns container flows whole, never splits, and its `fr` tracks resolve against the fixed panel width. See DECISIONS → "Structural columns + variable block sizing".
