...
**⏭ NOW: the teacher-grading slice is APPLIED, PUSHED, AND CI-GREEN. Nothing is queued for the author except the standing items below (gate 4, the D24 counsel read, the P8 boomerang).**

**✅ AUTHOR STEP 0 DONE (2026-08-16): 0034 applied, `verify:auth --target live` = 116 passed / 0 failed across 10 scripts.** (Live scores 116 where a fresh local DB scores 114/2 — the two extra passes are verify-image-storage and verify-0020, whose preconditions need a seeded published activity that live has and a reset local DB does not. The local "failures" were never defects.) **Independently re-verified against live, not against this file:** migrations through 0034 · `check_grades` present with RLS enabled AND forced, **zero policies**, no client table grants · `grades` + `can_grade_submission` GONE · all four RPCs present and anon-denied · `check_id` CASCADE + `graded_by` SET NULL · `grade.release` in the enum · the readback's body contains no `activity_versions` reference (the leak guard) · the upsert gates on `can_edit_activity` and never `can_read_activity`. Edge Functions unchanged: get-activity **v19** (`verify_jwt:false`) + check-activity **v15** (`true`) — read from the `version` field, not the `_11`/`_6` path suffix.

**✅ The purge rewrite has a LIVENESS proof, not just a source check (P3).** §G's whole reason for existing is that a purge job citing a dropped relation fails at its NEXT NIGHTLY FIRE, not at migration time — so waiting for 03:00 UTC to find out would have been the wrong kind of confidence. `purge_soft_deleted()` was executed end to end against live data inside an aborted transaction (the verify-0025/0033 EXPECTED-ROLLBACK idiom): it ran clean, and row counts after (8 activities / 24 versions / 3 users / 1 class) confirm nothing was durably deleted. ⚠ **Historical context for that table, so a future reader does not misread it:** jobid 1 FAILED on 2026-08-14 03:00 with a `submissions_activity_id_fkey` violation — that predates 0029's wipe the same day, and the 08-15 run succeeded. It is resolved, not lurking.

**✅ AUTHOR STEP 1 DONE — pushed 2026-08-16 (`origin/main` = `27986f8`), CI GREEN on its first run of the slice: [31893704928](https://github.com/ZanReed/activity-platform/actions/runs/31893704928).** The first push attempt silently did not land (origin stayed at `68e4d1e` while local was 7 ahead) — worth knowing as a failure mode, since the tree, the tests, and the live DB all looked correct while the deployed app was simply older than everything else. **`git ls-remote origin refs/heads/main` is the check that catches it**; `git status` alone would have said "ahead 7" either way.

**The run's numbers, and why each is the interesting one:** unit suites match local exactly (schema 340 / graph-kit 384 / viewer 1139 / app 1077) · **12/12 perf budgets** with the regenerated grading-server bundle passing both drift guards — the bundle rule earning its keep, since `wire.ts` feeds that bundle and a stale one fails CI rather than production · the four e2e lanes at **73 passed, 0 flaky**, up from 72, which is exactly the one a11y row this slice added (a count that did not reconcile would mean a phantom or a lost row).
