...
0. **✅ CI CLEAN — nothing pending here.** Run 31795715961: **71 passed, ZERO flaky**, all three jobs green; tree pushed and clean. The preceding green run (31791526509) had carried 2 flaky — not the row fixed the run before, but its two `two tabs` siblings holding byte-identical copies of the same snapshot-mid-settle idiom, so fixing one row had let the class surface one row over (the twin-drift lesson, in test code). All four rows now share one `settledLockHolders()` helper. ⚠ **Weak-evidence caveat, deliberately kept:** none of the three sightings ever reproduced locally, and one clean run is corroboration, not proof — the structural argument (the idiom is gone from all four rows) is the strong half. **Read the flaky COUNT, not just the conclusion, on the next few runs before treating this as closed.**
