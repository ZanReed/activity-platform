...
**⏭ NOW: nothing is blocked.** 0035 is applied live, `verify:auth --target live` = **127/0**, and `efd18c3` is pushed — **tool-confirmed, not claim-read**: `git ls-remote origin refs/heads/main` = local HEAD = `efd18c3`, and a live re-read shows 0035 in `schema_migrations`, the view present, `rolled_through` present with **0 rows ever written**, and `prune_section_checks()` returning `{"candidates":0,"deleted":0,"note":"no watermark — rollup has never run; prune inert (D11)"}` — **the arming gate is closed in production.** Everything below is a STANDING item with its own trigger, not a queue.
...
**Baseline facts, tool-read live 2026-08-16 (never claim-read):** migrations applied **through 0035** · exactly **TWO** Edge Functions — `get-activity` **v19** (`verify_jwt:false`, the only one) + `check-activity` **v15** (`true`) · `pnpm verify:auth --target live` = **127 passed / 0 failed across 11 scripts** · both pg_cron jobs active · live rows (re-read 2026-08-16 after the 0035 apply): **3 teachers, 1 student, 0 pending, 0 checks, 0 grades, 1 class (`7NE9M2`, 1 active member), 8 activities** · **0 `analytics_job_runs` rows carry a `rolled_through`** — the arming gate is closed, and `prune_section_checks()` says so in its own words. ⚠ **"No real student data exists yet" is NO LONGER TRUE** — one real second account is enrolled. It is the author's own throwaway, so the compliance answers stay cheap to change, but the sentence that made them free has expired. **Re-verify flags with `list_edge_functions` after every deploy — read the `version` field, NOT the `entrypoint_path` suffix beside it** (that misread produced a false "verified live" stamp twice; `get-activity`'s path ends `…_11` while its version is 19).

⚠ **One standing watch-note (the retry-log discipline):** the a11y **gap-2 keyboard row** flaked ONCE (run 31852826598) and has not recurred in the **eleven** green runs since (latest 31920975915, the 0035 slice: 73 passed / **0 flaky**). Instrumented in `e6b2872` so a second sighting names where focus actually is. **A second sighting is conclusive — fix then, not before. Read the flaky COUNT on every run, not the conclusion.**
...
| Database migrations 0001–0035 | ✅ **All applied + verified live — `verify:auth --target live` = 127/0 across 11 scripts** (0035 applied 2026-08-16; the gate re-confirmed by an independent live read of the function's own return). 0031+0032 were REPRODUCIBILITY migrations; **0033 is the admission slice**; **0034 is checks-native grading**; **0035 is the disarmed check-prune + arming gate**. Re-run `verify-0013-0014.sql` + `verify-0017.sql` after any auth/RLS/grant migration |
...
| CI (typecheck/lint/test/build + **2** bundle-drift guards + **12 perf budgets + 38 script tests** + print gates + perf/sw/student/**a11y** lane job) | ✅ **GREEN and CLEAN — run 31920975915 (the 0035 slice), 73 passed / 0 flaky.** **18** consecutive green runs, tool-counted (last failure 31787010974). Nothing is unpushed; the newest run covers HEAD |
...
**Last updated:** 2026-08-16 (late; 0035 applied live, 127/0, pushed, CI-green at 31920975915) — **The months-parked pruning/rollup entry closed in one session — as a DIFFERENT slice than it described.** Re-derivation (P10) → plan doc → /plan-eng-review (8 issues ruled interactively) → **the outside voice overturned the ratified frame**: "build the rollup now, prune disarmed" fell to "prune disarmed now, rollup at arming", because rebuild-from-raw makes the rollup rebuildable right up until the prune is armed — so building it against 0 rows was ~10 speculative DB objects. The synthesis that made the simpler frame safe (D11): the watermark COLUMN ships now, nothing writes it, and the prune refuses everything on NULL — the ordering invariant is schema, not prose.
