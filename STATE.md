...
**▶ Phase 2 — Reference panel COMPLETE + DEPLOYED (live on published pages, author-confirmed 2026-06-18).** Optional teacher-authored scaffold content (formula charts, vocab lists, conversion tables…). **A** (`76c40f7`): serialize round-trips `referencePanel` as editor state — closed a latent drop-bug. **B** (`0ba81bf`; renderer bundle regenerated): renders the panel — an on-screen collapsed bottom-bar toolbar (`<details>` + flex `column-reverse`, opens upward, zero runtime JS) + a print box at the worksheet top, gated by the new `meta.print.printReferencePanel` toggle. **C** (`9bd8b4d`): the authoring surface — an isolated, constrained `ReferencePanelEditor` (text + math + lists + columns + image; no sections, no blank-authoring UI) behind a collapsible "Reference panel" disclosure in the editor route, wired into autosave (fingerprint on the panel's Tiptap JSON, not the serialized panel). Reasoning in [DECISIONS](docs/DECISIONS.md) → "Reference panel". The **scoring** runtime stays untouched (the panel renders outside `.activity-section`); a separate ~1 KiB sidecar (`runtime/reference-panel.ts`, inlined only when a panel exists) adds author-requested **drag-resize** (top-edge handle) + **scroll-clearance** (a `ResizeObserver` pads the page by the panel's live height so the fixed bottom panel never permanently hides content), revising the earlier zero-JS call (see DECISIONS). Two bug fixes shaken out by the author's live testing (`f772222`, `83a40f4`): the body's content overflowed the page instead of scrolling — cap + scroll moved onto the **body** (`max-height:60vh; overflow:auto`), since `flex-shrink` doesn't constrain a child against a `max-height`-only flex *container*; and the resize handle sat above the bottom bar — handle + body are now wrapped in a `.reference-panel-content` flex column so the handle lands at the panel's **top** edge (the browser's `::details-content` wrapping made panel-level flex order unreliable). All green: schema 89 / renderer 280 / app 241, typecheck clean. **Deployed + live 2026-06-18:** author re-deployed `publish-activity` + re-published, confirmed working. **Follow-up done (2026-06-19):** removed the redundant `Underline` extension from `Editor.tsx` (StarterKit v3 already bundles it; standalone registration caused the benign "duplicate extension names" warning). Browser-verified on `/playground`: the duplicate-extension warning is gone, the live schema still carries the `underline` mark + `toggleUnderline` command (from StarterKit), and a toggle round-trip renders/unsets `<u>` with correct active-state. App-only change (no renderer/bundle impact); typecheck + 241 app tests green. One unverified-by-Claude nicety: the live drag *feel* (floor/ceiling/sensitivity) — verified by measurement, not eyeballed; tune if the author reports friction. Earlier: Phase 1 loop complete + verified end-to-end (order-independent blank groups shipped 2026-06-18 — see Recently completed). Other Phase 2 "decide at phase start" items still open: image-hosting quota, MC/matching/ordering UI, block-type priority, `skills` editing UI, custom R2 domain.
...
- **Vocabulary definitions / glossary (design captured Stage 9c-era; inline = Phase 2, shared store = Phase 4):** teacher selects text → "Define" → types an inline definition. An inline `DefinitionMark` (not a node) renders `<span class="definition" data-definition…>` (additive to the frozen data-attribute contract) with a runtime popover; Phase 4 adds a centralized glossary keyed by stable keys (`glossaryKey` attr name reserved now to avoid a later rename). **Scope decided 2026-06-19: account/tenant-scoped, not per-activity** — the glossary lives in a tenant-level `glossary_entry` table (not `ActivityMeta`), resolved at publish; promotion of existing inline definitions stays a scriptable additive transform. Design in [docs/design/vocabulary-definitions.md](docs/design/vocabulary-definitions.md). Predates the current toolbar / popover-host / print-CSS work — needs a short design refresh (toolbar affordance, print behavior, mobile/touch interaction) before coding; none of those touch the schema or data-attribute contract.
