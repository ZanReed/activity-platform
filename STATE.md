...
**Last updated:** 2026-07-13 (Phase 2.6 Slice 1 — `short_answer` + `essay` blocks — code complete, browser-verified, suite 1682 green. Reuse v9 `freeResponses` (no ingest redeploy); needs `publish-activity` redeploy. Free-text runtime generalized into `free-text.ts`. **Next: Phase 2.6 Slices 2–5 (the grading system) — start with a design pass on the rubric model + the 6 deferred ROADMAP decisions.**).
