...
3. **Board-UX pass (`3e12143`) changed the kit → re-upload + redeploy.** The 2026-07-05 upload shipped entry `EQ5MV42W`; the pan/zoom/resize fixes produce a NEW entry `graph-kit-KXEOKBPT.js` (+ chunk `DW43XIO2`). Re-run `pnpm build:graph-kit` with the R2 creds (same command that worked), then `supabase functions deploy publish-activity --project-ref dtqutpdplefmufrrakxs` (the deployed function still references the old hash until redeployed; the old files stay on R2 so nothing breaks in between). Then re-publish (action #2).
