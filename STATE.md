...
2. **Deploy the pedagogical-blocks train — ORDER MATTERS (wire bump).** `self_explanation` bumped the submission wire **v8 → v9** (the new `freeResponses` map), so the deploy must go **`ingest-submission` FIRST, then `publish-activity`** — a page publishing v9 POSTs a version the old ingest 400-rejects until ingest is redeployed. Use **`pnpm deploy:train`** (walks the safe order) or `pnpm deploy:ingest` (flag baked in) then the publish redeploy. This train also carries the render + dispatch for all four new blocks (`learning_objectives`, `worked_example`, `faded_worked_example`, `self_explanation`) — the deployed bundle drops them silently until `publish-activity` is redeployed. Ingest now accepts schemaVersion **3–9** (older pages keep POSTing their version). Storage bumped v9→**v10** (client-only, no deploy — old blobs discard to fresh state). Editor authoring rides the SPA deploy. `pnpm bundle:renderer` already run + committed (base runtime 38.3 KiB, graphs 45.1 KiB).
...
| Content blocks (`learning_objectives`, `worked_example`) | ✅ Code complete + browser-verified on `/playground` 2026-07-12. Pure content, no wire bump. **Needs `publish-activity` redeploy** (new bundle) to render on published pages. Import fences deferred to a fast-follow |
| Faded worked example (`faded_worked_example`) | ✅ Code complete + browser-verified on `/playground` 2026-07-12 (nested fill_in_blank NodeView renders; structure serializes). Faded steps = nested `fill_in_blank` → scored by existing runtime (new init test proves nested discovery), **no wire bump**. **Needs `publish-activity` redeploy**. Import fence deferred. Live author eyeball of `{{}}` blank-authoring inside the nested step is a nice-to-have |
| Self-explanation (`self_explanation`) | ✅ Code complete + browser-verified on `/playground` 2026-07-12 (prompt + preview + placeholder starter render/edit; serializes). Ungraded free text; **wire v8→v9 (`freeResponses`), storage v9→v10**. New runtime module (persist+gather), RUNTIME.md contract, dashboard display, ingest extended to v9. Base runtime 38.3 KiB (within budget). **Needs the ordered deploy train: `ingest-submission` BEFORE `publish-activity`.** Runtime persist/submit covered by jsdom tests; live student submit→dashboard round-trip is an author eyeball nice-to-have |

Suite now: schema 264 / graph-kit 325 / renderer 541 / app 526 (**1656**); typecheck + lint green (0 errors; 2 pre-existing warnings). The four pedagogical blocks added ~50 tests across the packages (incl. a runtime `init.test.ts` case proving a `fill_in_blank` NESTED in a `faded_worked_example` is discovered by `buildRefs`, and a jsdom `self-explanations.test.ts` for free-text capture/gather/persist). `runtime-doc-contract.test.ts` classifies the first three as CONTAINER and `self_explanation` as INTERACTIVE (its RUNTIME.md contract section is guard-enforced). Runtime bundle **base 38.3 KiB / graphs 45.1 KiB** minified (re-check with `pnpm bundle:renderer`; treat printed numbers as truth). Graphs is over the 40 KiB soft target — under the 60 ceiling, accepted per the 2026-07-10 budget amendment (DECISIONS.md → "Runtime size budget amendment"). **Base sits at 96% of its soft target** (self_explanation's runtime added ~0.8 KiB) — the next question-type generation should schedule the budget-ladder lever (per-question-type inlining variants; `document.ts` already picks by body scan) BEFORE it lands, per the amendment's "scheduled, not discovered" rule. The graph-runtime-into-kit lever was pulled 2026-07-10 — graph work grows the cached kit, not the inline bundles. (Environment note: this machine is the 2026-07-04 recovery clone — old computer died; toolchain re-set-up verified by a full green run. Kit-upload creds live in a gitignored `.env.r2`, auto-loaded by `pnpm upload:graph-kit` — the upload is an explicit command; `pnpm build:graph-kit` is build-only and never touches R2; see `.env.r2.example`.)
...
**Last updated:** 2026-07-12 (Phase 2 block-type family COMPLETE — all four pedagogical blocks shipped, ending with `self_explanation`; code complete, suite 1656 green, browser-verified on `/playground`, bundles regenerated + committed. Pending author action: run the ordered deploy train — `ingest-submission` (wire v9) BEFORE `publish-activity` — via `pnpm deploy:train`).
