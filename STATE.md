...
3. **Calculator (Phase 2.7): upload the graph-kit bundle to R2 + redeploy `publish-activity`.** Run `pnpm build:graph-kit` **with R2 creds in env** to upload `shared/graph-kit-<hash>.js` (~967 KiB / 264 KiB gz), then redeploy `publish-activity` (it now reads `_shared/graph-kit-manifest.ts` to form `calculatorKitUrl`). **Not blocking yet** — no teacher can enable a calculator until the editor authoring slice lands, so this can ride along with the #2 vocab redeploy. After the editor slice, this is required before a published calculator works. Current hash committed: `graph-kit-794f9f6fc52757ca.js`. MathLive fonts come from jsDelivr (no upload). See [supabase/functions/README.md](supabase/functions/README.md) → "Graphing kit asset on R2".
...
| Calculator tool (Phase 2.7, Stage 1 — scientific) | 🛠️ In progress: schema (`77ff8e3`) + renderer (`d7cf6cf`) + widget/`@activity/graph-kit` (`9f82383`, browser-verified) + build-hosting (`pnpm build:graph-kit` → R2 + manifest + publish-activity wiring) slices shipped + committed + tested. **Remaining: editor authoring control** (last slice). Pending author action #3: kit upload + publish-activity redeploy (not blocking until editor slice lands) |

Test counts at last session: schema 113 / graph-kit 26 / renderer 298 / app 245; `tsc -b` + app build green. (+ calculator: schema 12, renderer 8, graph-kit 26 tests.)
