...
3. **Deploy the comma-key kit change + Stage 5 (graded graph block)** — these ship together in one deploy pass:
   - **Kit re-hash** (comma key `dd5466f` + graph widget `f949c33`): `packages/graph-kit` changed → run `pnpm build:graph-kit` (upload FIRST), confirm the `Uploaded:` lines, note the new hash.
   - **`publish-activity` redeploy**: schema/renderer/runtime changed (new `interactive_graph` block + runtime spine) → `supabase functions deploy publish-activity --project-ref dtqutpdplefmufrrakxs`.
   - **`ingest-submission` redeploy — MUST use `--no-verify-jwt`**: now enforces submission `schemaVersion 3` (Stage 5 added `graphResponses`). Without the flag, anonymous student submissions 401 (see CLAUDE.md).
   - **Re-publish activities** (folds into action #2).
   - **Post-deploy verify (author):** on a real published page with a graph block, plot a point → check → confirm score + feedback, then submit → confirm the row + `graphResponses` land in the dashboard. (This full published-page path is the one thing Claude can't run in dev — the widget, runtime spine, and renderer are each verified in isolation; only the deployed lazy-kit-over-R2 wiring is unverified until this.)
...
| Interactive-graph block — Stage 5 slice 1 (2.7a plot-a-point) | ✅ Code complete (schema `db70beb` / renderer `db70beb` / kit widget `f949c33` / runtime `d3fd35a`); widget browser-verified at `/dev/graph-question`, runtime spine jsdom-verified. NOT yet on a live page — pending action #3 (kit re-hash + `publish-activity` + `ingest-submission` v3 redeploy + author published-page E2E). Next: slice 2 (editor NodeView) or 2.7b plot-a-line |

Test counts at last session: schema 136 / graph-kit 78 / renderer 316 / app 257 (787 total); `tsc -b` + app build + lint green. Runtime bundle 19.6 KiB (within the 20 KiB target — Stage 5 added the graph spine; watch this budget). (Environment note: this machine is the 2026-07-04 recovery clone — old computer died; toolchain re-set-up verified by a full green run. Deploy creds for `pnpm build:graph-kit` live in a gitignored `.env.r2`, auto-loaded — see `.env.r2.example`.)
...
- **Graph block — multi-point semantics gap (Stage 5).** The schema allows `correctPoints: [[..],[..]]` (min 1), but slice 1's WIDGET shows ONE draggable point and `isPointCorrect` matches "within tolerance of ANY correct point" (either-root). So an author who sets 2+ correct points today gets "plot either one," NOT "plot both." A real "plot both roots" question needs N draggable handles + consume-once matching (like blank groups). **Don't advertise multi-point authoring until the widget grows N handles** — the schema is ready, the widget isn't. Decide at 2.7b/2.7c or when a curriculum needs it.
...
- **Dashboard doesn't render `graphResponses` yet (Stage 5).** Submissions store + migrate the graph map (v3), and the score reflects it, but the Submissions dashboard only renders blanks/checkpointResults — a teacher sees the graph's score, not "student plotted (3,4) vs answer (3,4)." Add a graph-response view (plotted point vs answer key) as a slice-2 follow-up or when real graph submissions exist.
- **Runtime bundle budget trend (Stage 5).** Inlined runtime is now 19.6 KiB / 20 KiB target (40 KiB ceiling). Slice 2 is editor-only (no runtime add), but each new graded graph variant (2.7b line, 2.7c region) adds runtime plumbing (init/state/gather/persist — NOT scoring, which is pure + kit-side). If it approaches the ceiling, the lever is moving the graph runtime plumbing INTO the lazy kit (already loaded on graph pages) so the base runtime stays lean. Keep the scorers pure + DOM-free (`graph-score.ts`) so Phase 5 server-grading — which should strip `data-graph-answer-key` from graph HTML first — is cheap.
...
**Last updated:** 2026-07-06 (Stage 5 slice 1 — graded interactive-graph block, plot-a-point). Completed-session narratives are archived in [HISTORY.md](docs/HISTORY.md) (rolling log, newest first); durable reasoning in [DECISIONS.md](docs/DECISIONS.md). Suite at last session: schema 136 / graph-kit 78 / renderer 316 / app 257 (787); CI green locally (typecheck/lint/test/build).
