...
3. **Calculator (Phase 2.7): upload the graph-kit bundle to R2 + redeploy `publish-activity`.** Run `pnpm build:graph-kit` **with R2 creds in env** to upload `shared/graph-kit-<hash>.js` (~967 KiB / 264 KiB gz), then redeploy `publish-activity` (it now reads `_shared/graph-kit-manifest.ts` to form `calculatorKitUrl`). **Now required** — the editor authoring slice has landed, so a teacher can enable a calculator; without the upload + redeploy, a published page's summon button just fails to load (graceful: rest of page works). Can ride along with the #2 vocab redeploy. Current hash committed: `graph-kit-794f9f6fc52757ca.js`. MathLive fonts come from jsDelivr (no upload). See [supabase/functions/README.md](supabase/functions/README.md) → "Graphing kit asset on R2".
...
**▶ Phase 2.7 — Graphing track · Stage 1 (scientific calculator): CODE-COMPLETE 2026-06-21.** All 5 slices committed (schema `77ff8e3` · renderer `d7cf6cf` · widget `9f82383` · build-hosting `f581685` · editor `cfb8f04`); full narrative in [HISTORY 2026-06-21](docs/HISTORY.md). A teacher enables + restricts an on-screen scientific calculator per activity (editor 🧮 disclosure + live preview); students summon it while working (lazy-loaded from R2 on first click); it's an **ungraded scaffold** (never scored, no submission). New **`@activity/graph-kit`** package — MathLive + math.js behind an engine-agnostic `evaluate()` seam (browser-verified via the dev-only `/dev/calculator` harness; the evaluator's AsciiMath→math.js glue is unit-pinned). Decisions in [docs/design/calculator-tool.md](docs/design/calculator-tool.md). **To go live: author action #3** — `pnpm build:graph-kit` with R2 creds (upload the kit) → commit the manifest → redeploy `publish-activity`. ⚠️ Editor sidebar UI not yet browser-loaded (auth-gated route); composed of verified pieces (widget + serialize round-trip + typecheck) — eyeball in a logged-in session. **Next: Stage 2** — single-function graphing (JSXGraph board layer); the kit's `evaluate()` + (Stage 3) data-table/regression pieces stay standalone so the graded interactive-graph block + the noted **graded-regression** variant reuse them. Suite: schema 113 / graph-kit 26 / renderer 298 / app 250.
...
| Calculator tool (Phase 2.7, Stage 1 — scientific) | ✅ Code-complete (all 5 slices committed: schema `77ff8e3`, renderer `d7cf6cf`, widget `9f82383`, build-hosting `f581685`, editor `cfb8f04`); widget browser-verified, editor sidebar verified via typecheck + serialize tests (UI not yet auth-loaded). ⏳ Pending author action #3 (kit upload + `publish-activity` redeploy) to reach published pages |

Test counts at last session: schema 113 / graph-kit 26 / renderer 298 / app 250; `tsc -b` + app build green. (+ calculator: schema 12, renderer 8, graph-kit 26, app serialize 5 tests.)
...
**Last updated:** 2026-06-21. Completed-session narratives are archived in [HISTORY.md](docs/HISTORY.md) (rolling log, newest first); durable reasoning in [DECISIONS.md](docs/DECISIONS.md). Suite at last session: schema 113 / graph-kit 26 / renderer 298 / app 250; CI green.
