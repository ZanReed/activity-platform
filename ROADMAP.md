...
**User-visible**: Allowlisted teachers sign in via Google. They create activities in a vertical-stack block editor (paragraph, heading, math, image, callout, problem, fill-in-blank, bullet/ordered list). They publish to a static URL. They share that URL with students, who fill out the worksheet and submit with their name. Teachers see submissions in a simple table.
...
**Common question types beyond fill-in-blank**: multiple choice (single and multi-select), matching pairs, ordering / sequencing. These are basic question types in nearly every subject — and historically far more common than fill-in-blank in non-STEM classrooms — but they don't fit `BlankResponse` because the answer isn't a string. Each new type follows the parallel-map pattern established in the interactive-graph design doc: a new `choices` / `matches` / `orderings` map on `SubmissionResponses` (Submission response bumps to v3), a new block type in the discriminated union, a new scoring strategy in the `evaluateAnswer` dispatch (`'mc-single'`, `'mc-multi'`, `'matching'`, `'ordering'`), and a new NodeView. Architecturally similar to graphs but simpler since the response shape is just a selection / arrangement, not geometric coordinates.
...
Status: shipped ahead of schedule in Phase 1 (Stage 9e) — the marks are in the schema Mark enum, the serialize layer, and the toolbar. Kept here only as a record; no longer Phase 2 scope.
Subscript / superscript marks add two new entries to the mark schema, two new toolbar buttons... → Subscript / superscript marks (done in Stage 9e) added two entries to the mark schema, two toolbar buttons, two Tiptap mark extensions, and serialize support.
