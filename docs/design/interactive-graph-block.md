...
> ⚠ **INFRASTRUCTURE ANNOTATION (drift audit 2026-08-22).** This doc names `packages/renderer/RUNTIME.md`'s data-attribute contract, a graph kit hosted on R2, and the runtime's `graph-integration.ts` seam as live. All of them died at S9 (2026-08-14) — `packages/renderer` and `RUNTIME.md` at Drop 4 — and the R2 kit upload at the D-13 teardown (2026-08-15). The viewer (`packages/viewer/src/blocks/`, opened by students at `/a/:id`) renders the block now, the graph kit builds from the workspace into an app-bundled lazy chunk, and grading is server-authoritative in `check-activity`. The text below is left intact as the record of how the feature shipped.

**Status:** ✅ **SHIPPED** (Stage 5, 2026-07; live-verified 2026-07-10) **and still growing — the live interaction roster is the registry's `variants` list (`packages/viewer/src/registry/registry.ts` → `interactive_graph`), not any count pinned here** *(this line said "four graded interactions" until the 2026-09-02 audit; `transform_curve` was the fifth, and the function families have since grown cubic/quartic/absolute/sqrt — [graded-function-families.md](graded-function-families.md), [draggable-curve.md](draggable-curve.md))*. At this doc's writing: plot a point (N-handle), plot a function (incl. rays/segments with endpoint gliders), graph an inequality, shade a region, plus an ungraded static `display` mode. The Zod definitions live in `packages/schema/src/blocks/interactive-graph.ts`; `packages/renderer/RUNTIME.md` holds the current data-attribute contract.
