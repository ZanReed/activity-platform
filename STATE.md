...
| S8 perf-budget CI | route split (entry = student shell), 12 CI size budgets + 23 script tests, throttled perf lane, sw/student lanes into CI, unified budget config | ✅ 2026-08-05 — T7 KaTeX call + timing calibration open (above) |
...
2. **T7 (KaTeX) is decision-ready.** The measurement it waited on exists: KaTeX is 75.2 KiB gz and the **LaTeX-fallback window is ~740 ms**. TODOS.md now carries three options, not two — the S8 outside voice added *preload-on-math-detect*, a fetch-timing change that needs no D16 amendment and spends no shell bytes. Recommended to try that first and re-measure.
