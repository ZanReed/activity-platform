...
> ⚠ **INFRASTRUCTURE ANNOTATION (drift audit 2026-08-17).** The block itself is live and correct.
> But the delivery mechanisms this status line names — the **R2-hosted graph kit**, **`ingest-submission`**,
> **`publish-activity`**, **`STORAGE_SCHEMA_VERSION`**, and the wire-version ladder — were all
> **DELETED at S9 (2026-08-14) and the R2 bucket torn down at D-13 (2026-08-15)**. Published HTML
> pages no longer exist; the student surface is the viewer at `/a/:id`, the graph kit is an
> app-bundled lazy chunk built from the workspace, and grading goes through `check-activity` +
> `record_check`. Read the kit hashes, function versions, and wire/storage numbers below as
> **historical record of how it shipped**, never as current deploy state. See CLAUDE.md's S9
> tombstones.

**Status:** ✅ SHIPPED + DEPLOYED (all 6 slices; 2026-07-12) — kit `WXYPUC5W` on R2, `ingest-submission` v35 (wire 3–8, `verify_jwt:false`) + `publish-activity` v86 live; author-confirmed. The graded builds (`build_histogram` + `build_boxplot`) also shipped + deployed (kit `V6T6HAUS`, ingest v36, publish v88) — see the follow-up section below. All §3 decisions resolved = the recommended answers (1b display + build_dotplot; 2 own `dataPlotResponses` map + wire v7→v8 + storage v8→v9; 3a compute the plot from `data`; 4 TI-84 exclusive-median for box display, per-block method deferred to box-build; 5 reuse `NumberLineConfig` + histogram extras; 6 dedicated SVG chart widget in the kit; 7 print/no-JS SVG yes; 8 all-or-nothing; 9 defer the ```dataplot``` fence). **Built + committed (6 slices):** schema + wire v8 + ingest 3–8 (`96e3da6`), static SVG renderer + print/no-JS (`96e3da6`), pure `scoreDotplot` + SVG dot-plot board + `mountDataPlotQuestion` (`ee9a5bc`, browser-verified at `/dev/data-plot`), the runtime bridge (`dataPlotExt` seam + `attachDataPlotRuntime` + storage v9 + wire v8 + variant scan; `e2f8a39`, browser-verified on a **bundle-rendered** page: display box plot + build→check→"1/1 correct"→reload-restore, zero console errors), the editor NodeView + data-table authoring + serialize round-trip (`39c26c6`, `/playground`-verified: live preview + mode switch), and the dashboard "Data plot" table (`bd26f19`). As-built deltas from the plan: **no author board** (a data_plot is authored by editing its dataset + a static preview, so there is no `mountDataPlotAuthor` — the plot is computed from `data`, decision 3a); **display is pure static SVG** (no kit hydration — dot/histogram/box are fully static-drawable, unlike graph `expression` drawables), so display data plots stay on the leaner base runtime and only a GRADED data_plot triggers the graphs variant. Follow-ups (§4): `build_histogram` + `build_boxplot` ✅ SHIPPED (the follow-up pass below); the ```dataplot``` import fence ✅ SHIPPED 2026-07-12 (fast follow — importer + Copy-AI prompt + docs/markdown-import-format.md + drift guards); remaining: unequal-bin `binEdges`, per-bin partial credit.
...
Follow-ups (post-slice-1): `build_histogram` + `build_boxplot` ✅ SHIPPED (follow-up pass below), ```dataplot``` import fence ✅ SHIPPED 2026-07-12, unequal-bin `binEdges`, per-bin partial credit, custom tick labels.

---

# Follow-up design pass: graded histogram + box-plot builds

**Status:** ✅ SHIPPED + DEPLOYED (all 5 GB slices; 2026-07-12) — kit `V6T6HAUS` on R2, `ingest-submission` v36 (widened `DataPlotResponse` union, `verify_jwt:false`, MCP-verified) + `publish-activity` v88 live; author-confirmed. **NO wire/storage bump** (additive union members). All §B decisions = the recommended answers: 1a both builds (histogram → box), 2 NO wire/storage bump (additive union members), 3a draggable bar tops, **4 box-plot method LOCKED: TI-84 exclusive-median + `tolerance` 0.5 default**, 5 histogram marker / box `tolerance`, 6 all-or-nothing, 7 picker + preview (no author board), 8 per-variant dashboard formatting, 9 deferred the ```dataplot``` fence. **Built + committed (5 GB slices):** schema + response union `e6bb062`, kit scorers + SVG histogram/box boards `58fbee6` (browser-verified at `/dev/data-plot`), renderer + runtime gather `2b32bce` (bundle-rendered-page-verified: histogram [2,3] + box [1,2,4,6,7] → "2/2 correct"), editor picker + box readout `19769c4` (`/playground`-verified: box preview + TI-84 readout, histogram bars), dashboard per-variant formatting `913744d`. Suite: schema 244 / graph-kit 325 / renderer 516 / app 468 (1553). Extends the SHIPPED block above (slice 1: `display` + `build_dotplot`).
...
**Decision 9 — ```dataplot``` markdown import** — still **defer** (out of scope; same as slice 1 decision 9). *(✅ SHIPPED 2026-07-12 as the fast follow: `answer:`/`show:` mode lines mirroring the graph fence, auto-fit axis default, lean config — axis `step` doubles as the histogram bin width, box tolerance as an answer-line suffix. Author approved all three syntax decisions. Spec in [markdown-import-format.md](../markdown-import-format.md).)*
