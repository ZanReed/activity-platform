...
1. **Free activity catalog / "Activity Bank"** (Phase 2 slice: browse + run/assign/print; [docs/design/free-activity-catalog.md](docs/design/free-activity-catalog.md)) — the onboarding/cold-start lever. **Design-reconciliation pass DONE 2026-07-13** (new "Design reconciliation" section in the doc): architecture holds; `visibility` enum + inert marketplace columns mean no new schema; two security corrections (don't widen `can_read_activity` — it gates submissions; no row policy on `activities` — draft_content leak; use a definer RPC) and a consumer-print fork (full engine on published-version-only vs. browser-print of the R2 page). **7 numbered decisions await author yes/no before code.**
