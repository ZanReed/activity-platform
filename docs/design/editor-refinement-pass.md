...
1. **Blank discoverability.** A visual signifier that you can make a blank.
   **RESOLVED via `/plan-design-review` 2026-07-19 → FORM A (ghost text only):**
   inline ghost text via the existing fill-in-blank `PromptField`
   ...
   blank exists). Author dropped the hybrid's second leg (a quick-bar "Insert
   blank" action) as unnecessary. Rejected the standalone `+`-underline in-text
   widget: its `+` collides with the gutter insert-`+`, its underline mimics a
   real blank, and it adds a new a11y surface — off-idiom for an editor whose
   one "what can I put here" language is faint ghost text (the `/` hint,
   PromptField). Full spec in [math-blanks.md](math-blanks.md) T7.
