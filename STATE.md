...
2. **`publish-activity` redeploy — reference panel (Drop B).** The renderer bundle now emits the reference panel: an on-screen collapsed bottom-bar toolbar (`<details>`, zero runtime JS) + a print box at the top of the worksheet, gated by the new `meta.print.printReferencePanel` toggle (default on). `pnpm bundle:renderer` regenerated + committed. Redeploy so published pages can render a panel. **Not urgent** — backward-compatible (panel-less activities render byte-identical), and no activity can carry a panel until the editor authoring surface lands (Drop C). No DB/migration; no `ingest-submission` change.
...
**▶ Phase 2 — Reference panel in progress (Drops A + B done; Drop C next).** Building the optional teacher-authored reference panel (formula charts, vocab lists, conversion tables…). **Drop A** (committed `76c40f7`): serialize round-trips `referencePanel` as opaque editor state — closes a latent drop-bug where any stored panel was discarded on the next save. **Drop B** (committed this session; bundle regenerated): the renderer emits the panel — an on-screen collapsed bottom-bar toolbar (`<details>` + flex `column-reverse`, opens upward, zero runtime JS) + a print box at the worksheet top, gated by the new `meta.print.printReferencePanel` toggle (surfaced in editor Print settings). Verified live in the browser (bottom sheet opens upward, KaTeX renders, collapsed by default). Runtime untouched: the panel renders OUTSIDE `.activity-section`, which is the only thing the init walker scopes to. **Drop C (next):** the editor authoring surface — a constrained second editor (content blocks incl. columns + image; no sections/blanks) behind a disclosure in the editor route. Design settled with the author: screen = bottom bar, print = top box, drag-to-reposition deferred, on/off print toggle (granular later). **Pending:** redeploy `publish-activity` (above — not urgent until Drop C). Decisions writeup for DECISIONS → "Reference panel" is owed. Earlier: Phase 1 loop complete + verified end-to-end (order-independent blank groups shipped 2026-06-18 — see Recently completed). Other Phase 2 "decide at phase start" items still open: image-hosting quota, MC/matching/ordering UI, block-type priority, `skills` editing UI, custom R2 domain.
