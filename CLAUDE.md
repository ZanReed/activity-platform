...
- **docs/design/curriculum-alignment.md** — the catalogue's organization contract: declared identity (`key:`), chains, skill coverage, and the validator split with the curriculum side. Read before touching `scripts/batch-import.mjs`. ⚠ **The catalogue is authored by a SEPARATE curriculum-side agent** with its own repo, decision log and skill graph; this repo never reads that graph and consumes only the registries generated from it. Their `boundary-page.md` is the shared surface (open items, correspondence index, retractions) — §5e records what this side retracted, because it otherwise lives only in their file. **If a file you do not own looks wrong, file it — do not fix it.**
...
- **After changing `packages/app/src/lib/catalogueAuthoringPrompt.ts` or the meta fence it teaches, run `pnpm prompt:catalogue` and commit the regenerated doc.** `catalogueAuthoringPrompt.test.ts` fails on drift. That file is the curriculum builder's single source — it exists to retire the hand-kept copy that is the known weak point of that system, so a stale copy here recreates exactly what it replaced. **The cross-side pointer to it is a row on the platform-owned Notion boundary page** (<https://app.notion.com/p/3ceb20a1fdd28183a9afcc7e19d905c1> — one-way: this side writes, curriculum side reads; no content is ever restated there, only the GitHub `main` URL + a generation stamp). After a regeneration lands on `main`, refresh that row's stamp (commit + sha256) — the pointer itself targets `main`, so only the stamp can go stale, never the doc.
