...
**⚠ APPLY migration `0024_audit_actor_purge.sql`** (`supabase db push`) — the audit-clock ruling: `audit_log.actor_id` → ON DELETE SET NULL, plus a `metadata.actor_purged` stamp so the event survives its 2-year window without naming a person. **This is what makes account deletion possible at all** — until now every account was blocked by its own `user.create` row. Proven end to end in a rolled-back transaction (account owning nothing deleted cleanly, audit row survived + stamped, null actors 7→8); nothing committed. No redeploy (pg_cron calls it in-DB). After applying, run `scripts/verify-0024.sql` — **section 0 first**, and note section C reports a PASS as a `P0001` error.
...
- **⚠ ONE GAP LEFT before retention is real: nothing sets `users.deleted_at`, so the account clock never starts.** Both retention rulings landed 2026-08-04 — the account waits for its work (0023) and a purged account's audit events survive without naming a person (0024) — and the purge is now mechanically **capable** of completing, proven end to end against the live DB (an account owning nothing deleted cleanly, its audit row surviving and stamped). But no soft-delete-student flow exists, so nothing ever marks an account departed and the job stays inert in practice. **This is the last item between the retention policy's windows and reality; it must close before the first real student account.** Lands with the S4/S7 purge-job work. Context in [retention-policy.md](docs/compliance/retention-policy.md) → Mechanics, which counsel must read alongside the windows table.
...
**Last updated:** 2026-08-04 — **three migrations shipped, all applied and verified: 0021 (display_name privacy), 0022 (purge-job section_checks fix), 0023 (account retention clock).** A privacy fix opened the thread; refreshing the compliance pack to match it exposed that the retention machinery underneath was broken in three ways, two now fixed. **The one that remains is the important one: no account can be purged at all** — every account is blocked by its own `audit_log` trail, and nothing starts the account clock. Both are named under Standing constraints and in [retention-policy.md](docs/compliance/retention-policy.md) → Mechanics; they must close before the first real student account. Every fix was reproduced against the live database inside rolled-back transactions before being trusted. Trigger stops storing emails, anon meta RPC refuses to return them, 2 rows backfilled to NULL; name-appearance control backlogged. Full verification pass green including a live anonymous HTTP fetch (detail under Pending author actions). One decision left open there: a third account still publishes the real name "Zan Reed" — intended behavior, but worth an explicit call. Earlier same day: drift audit + STATE restructure (9 mechanical findings fixed, finished narratives to [HISTORY.md](docs/HISTORY.md)); suite verified green (counts above).
