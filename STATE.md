...
5. **▶ LAST STEP — paste `scripts/verify-analytics-e2e.js` into the browser console** at https://activity-platform.pages.dev while signed in as a teacher, then open `/activity/<id>/analytics`. This is the only remaining unverified leg, and it needs a real teacher session (Supabase keeps it in `localStorage`, so it cannot be driven from outside your browser).

   **Already verified without a session (2026-08-05, plain curl — the verify-check-e2e precedent):** the META branch answers **200 anonymous** with `teacher_name:null` (so get-activity v8's `--no-verify-jwt` is live AND 0021's privacy fix still holds); the CONTENT branch **401s** without an `Authorization` header; and `get_activity_analytics` as anon is refused by the GRANT — `42501 permission denied for function` — which is probe leg C2. The **deployed frontend** carries the panel: Pages rebuilt on the push, `/activity/:id/analytics` is a registered route, and `get_activity_analytics` / "Activity analytics" / "Not yet attributed" all appear in the served bundle. What the console paste adds that none of this reaches: an authenticated OWNER reading the RPC over real HTTP, an authenticated NON-owner being refused there (the database gate itself is already proven — verify-0026 C7/C8), and the panel rendering real numbers.
