...
**Adding a new block type is mechanical, but it touches every package, in this order.** Schema side: a new file under `packages/schema/src/blocks/`, registered in `blocks/index.ts`'s discriminated union, then a factory in `packages/schema/src/factories.ts`. Renderer side: a new file under `packages/renderer/src/blocks/`, registered in `blocks/index.ts`'s dispatch switch (the `never` exhaustiveness check won't compile until you add it), plus matching styles in `packages/renderer/src/runtime/styles.ts`. Editor side: an entry in `packages/app/src/editor/slashMenuItems.ts`, and a Tiptap extension (plus a NodeView for blocks that render interactively, like math) under `packages/app/src/editor/`. The pattern in `problem.ts` / `fill-in-blank.ts` (schema and renderer) and `MathInline` (editor) is the canonical reference; STATE.md's standing constraints keep the authoritative checklist.
...
This README intentionally does not track build status — that is what `STATE.md` is for, and the previous version of this section rotted precisely because it tried to. For the current stage breakdown and what's in flight, read `STATE.md`; for the long-term phase plan, `ROADMAP.md`. In broad strokes: the Phase 1 foundation is in place — schema, renderer, the Tiptap editor (`@activity/app`), and the `publish-activity` + `ingest-submission` Edge Functions — with editor-to-Supabase wiring and the published-page runtime as the active work.

## Edge Functions

The `supabase/functions/` directory holds Deno Edge Functions. `publish-activity` takes a draft, atomically snapshots a version, validates, renders, and uploads the static HTML to Supabase Storage. `ingest-submission` receives student submissions from published pages, validates them, and writes to the `submissions` table. See `supabase/functions/README.md` for setup and deploy instructions.
