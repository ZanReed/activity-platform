...
**Session ledger 2026-08-06→07 (all pushed unless noted):** eng review (28 rulings, 0 unresolved) → 41-item batch (9 commits) → CI un-redded (env-baked dist + math shutter + polled @page + 2× baseline regens) → B5 print heights → D2/D3 compliance rewrite + first POLICY_VERSION bump (author-approved) → local-first slice `3cd4a7e` (CI green) → **B8 malformed-document slice (2026-08-07, unpushed)**: walk integrity gate landed red-green (30 red → 40 green in `walk-integrity.test.ts`, cases in corpus.ts's `INTEGRITY_CASES`) — the rule: a grader-read field PRESENT with a shape the schema cannot author throws `MalformedDocumentError` → 500 + `malformed_document`; absent/authored-empty grades as before. Client maps the code to its own non-retryable kind + copy (D29 — it would otherwise land in retryable `server_error`). Census opts out via `{integrity:'coerce'}` (read path = withhold-and-serve, behavior-neutral). Both server bundles regenerated. Known nuance, documented in walk.ts + the handler: upgrade-time Zod means no STORABLE document trips the gate today — it is the engine's own contract (defense in depth behind the `as never` cast), so the handler branch's liveness proof uses the test file's one pass-through mock (P3). **Next in ruled order: (1) the S9-prep identity slice** (B12/B13/B14 rulings — role in SessionContext, student Home branch, join-by-code + /join/:code link, gate hardening migration FIRST, two-action remove dialog; BEFORE student_domain seeding), **then (2) S9 gets its own /plan-eng-review before build.** Unblocked opportunistic: the D23 privacy-hash TODO.
...
2a. **⏳ B8 deploy (2026-08-07): redeploy `check-activity` via `pnpm deploy:check`** (NO `--no-verify-jwt` — it stays gated) to bring the malformed-document gate + `malformed_document` wire code live. Grading-server bundle is regenerated and committed with the slice. This deploy is also the one that closes A2 above. *(The viewer-server bundle also regenerated — census now passes an explicit `{integrity:'coerce'}`, behavior-neutral — so get-activity's next deploy, whenever it happens, carries it with no ordering constraint.)*
