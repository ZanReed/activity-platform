...
- **⚠ Retention still has ONE open decision (0022 fixed the mechanical half).** `submissions.student_id` is ON DELETE RESTRICT, so purging a student who has account-backed submissions still fails: the account's 30-day clock and the submissions' 400-day clock disagree and **nobody has ruled which wins**. Harmless today (zero account-backed submission rows), and deliberately left alone in 0022 because picking an answer silently would bake in a retention policy no one chose. Decide it with the S4/S7 purge-job work. Context in [retention-policy.md](docs/compliance/retention-policy.md) → Mechanics.
