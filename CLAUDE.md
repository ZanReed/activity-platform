...
- **The anonymous student-facing functions must always be redeployed with `--no-verify-jwt`: `ingest-submission` AND `get-feedback`.** Published pages call both from the runtime with NO Authorization header, so the platform's default `verify_jwt` gate 401s every request before the function runs. There is no `config.toml`; the flag lives only on the Supabase platform and a plain redeploy silently re-enables JWT verification (this bit get-feedback on its first deploy — advisor/`list_edge_functions` caught `verify_jwt:true`). Use `pnpm deploy:ingest` / `pnpm deploy:feedback` (flag baked in) instead of a raw `supabase functions deploy`; `pnpm deploy:train` walks a full multi-part deploy in the safe order. (`publish-activity` and `upload-image` are teacher-only and stay `verify_jwt:true`.)
