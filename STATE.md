...
- **Markdown import — complete (core + math + images)** (2026-06-16) — paste-markdown → editor blocks. Standalone converter [`lib/markdownToTiptap.ts`](packages/app/src/lib/markdownToTiptap.ts) (markdown-it, lazy-loaded + vanilla) + a header **Import dialog** with a live block/problem summary and warnings. Covers headings, paragraphs, bold/italic/code, nested lists, blanks→`fillInBlank`, `{checkpoint}`→checkpoint section break, `$…$`/`$$…$$` math, and `![alt](url)` images (lifted out of the paragraph into image blocks); a blank-bearing list flattens to one problem per item. Blanks/`{checkpoint}` resolved in the mapper; **math lifted from raw source before markdown-it** so LaTeX backslashes survive CommonMark escaping (the lean-core pass shipped this without math; the fast-follow added math+images and the raw-source fix). **App-only/additive — no schema/renderer/runtime change, no bundle, no deploy.** 37 converter tests (incl. schema round-trip + LaTeX-fidelity + code-span guard); verified twice against the live ProseMirror schema on `/playground` (all shapes accepted, blanks render as chips, math KaTeX-renders with backslashes intact, image block carries src+alt). Decisions in DECISIONS → "Markdown import".
...
- **Markdown import — complete (core + math + images)** — standalone `markdownToTiptap.ts` (markdown-it, lazy + vanilla; math lifted from raw source so LaTeX backslashes survive) + header Import dialog. App-only/additive: no schema/renderer/runtime change, no bundle, no deploy. 37 converter tests + live-editor verification on `/playground` (math KaTeX-renders, images carry src+alt). **Dialog UI bug found + fixed:** the modal vanished the instant it opened — FocusTrap `onDeactivate: onClose` fires on React StrictMode's dev double-unmount; fix decouples close from the trap's lifecycle (Escape keydown + backdrop mousedown only). Reproduced/verified via a temp playground harness. Decisions filed in DECISIONS → "Markdown import".
...
- Suite: schema 54 / renderer 254 / app 195; CI green. Markdown-import work on branch `feature/markdown-import` (3 commits, unpushed).
