...
| **2** | Polish the loop | ~~Markdown import~~ (shipped early in Phase 1), ~~image upload~~, ~~reference panel~~, ~~vocabulary definitions~~, ~~multiple choice~~ + ~~numeric blanks~~ (2026-07-10), ~~matching + ordering~~ (2026-07-10), ~~number-line + data-plot question blocks~~ (2026-07-12), better submission analytics, self-signup |
...
> **⚰️ RETIRED at S9 Drop 3 (2026-08-14, ruling OV-5).** The Submissions dashboard WAS the grading UI and died with the anonymous wire; the student-feedback half (get-feedback) never worked live (P9 audit — bodiless 200s from birth). What survives: the short_answer/essay blocks + rubric authoring (editor/schema/viewer) and the empty `grades` table. The **parked teacher-grading slice** owns the successor, bound to `section_checks`; trigger unchanged — the first real teacher. The section below is design history for that slice to mine, not shipped behavior.
