# Misconception binding manifest

<!--
  GENERATED — do not hand-edit. Refreshed by `pnpm import:batch`
  (a --dry-run is enough; it writes this file in both modes).

  THIS IS AN AUTHOR-REFRESHED SNAPSHOT, NOT A CI-GATED ARTIFACT.
  The .md files it summarizes live OUTSIDE this repository (the
  author's catalogue folder), so CI cannot regenerate it and no
  drift check against it could ever pass. Do not add one. It is
  committed so that binding changes are reviewable as a diff —
  that is the whole job, and a stale snapshot is a stale snapshot,
  not a build failure.
-->
