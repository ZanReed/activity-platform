...
**Status:** design captured, not implemented. This document holds the shape decided in a Stage 9c-era conversation so that whoever picks up Phase 2.7 (probably future-me) has the rationale alongside the code shape. When Phase 2.7 lands, the Zod definitions migrate into the schema package as real exports and this doc gets either deleted or moved to an `archive/` subdirectory.
