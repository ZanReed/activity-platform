...
> ⚠ **INFRASTRUCTURE ANNOTATION (drift audit 2026-08-22).** This doc names a graph kit content-hashed on R2 and lazy-loaded from there, plus `STORAGE_SCHEMA_VERSION` as live. The R2 kit upload died at the D-13 teardown (2026-08-15) and `packages/renderer` (with `STORAGE_SCHEMA_VERSION`) at S9 Drop 4 (2026-08-14). The kit is now an app-bundled lazy chunk built from the workspace and summoned inside the viewer at `/a/:id` — no upload step exists for a kit change. The text below is left intact as the record of how the feature shipped.

**Status:** ✅ **LIVE FOR STUDENTS AGAIN since 2026-08-23** — the summon was rebuilt in the viewer as the tool cluster ([floating-tool-cluster.md](floating-tool-cluster.md), plan + AS-BUILT); `ToolCluster` reads `ActivityDocument.calculator` and mounts the same `mountCalculator()` the editor preview uses. *(It was SHIPPED 2026-07 and then NOT REACHABLE BY STUDENTS from S9 Drop 4 to 2026-08-23 — the old summon lived in the published-page runtime and died with it; the 2026-08-22 drift audit caught it as orphan #4.)* **The FEATURE SCOPE was ruled before the wiring** (2026-08-23, against ground truth read from `packages/graph-kit/src` rather than from this doc): clickable intersections and intercepts are **OUT** — they do not exist and are a solver + interaction-model arc on the Tier B side of the pedagogy line — and cross-row definitions ship as the **minimum** (`a = 10` then `a*2` reads `= 20`, no dependency graph). See DECISIONS.md → "Calculator feature scope". Shipped earlier: Stages 1–4 (scientific → single-function graphing → data table + regression → multi-expression list) plus the 2026-07-06 graphing UX overhaul (floating draggable panel, in-panel MathLive keyboard, splitter, hover-trace, sticky axes) and the 2026-07-11 parity batch. The kit is `packages/graph-kit`, now an app-bundled lazy chunk (not R2); the graded face ([`interactive-graph-block.md`](interactive-graph-block.md)) shipped on the same kit — "one track, two faces" held. Later stages (stats beyond regression, per-section overrides, calculator state persistence) remain unscheduled. The rest of this doc is the original 2026-06-19 rationale — still the reference for the track's decisions; where detail conflicts with code, code wins.
...
| 3 | **Data table + regression** (linear / quadratic / exponential) — **SHIPPED 2026-07-04** | Table component + least-squares engine. ← Algebra I requirement |
| 4 | **Multi-expression list** — **SHIPPED 2026-07-05** | N rows, sliders, points, color coding — the Desmos-defining surface. |
...
5. **Regression UI → a Data view inside graphing mode (Stage 3, 2026-07-04; author green-lit over a third top-level mode).** A `Data` header toggle flips the panel to a two-column layout (table + model dropdown + aria-live results beside the board) — the field/keypad hide, the board stays. CSS-only view switch (no DOM reparenting under JSXGraph); pre-figures Stage 4's expression-list layout; the `scientific` mode ceiling excludes it without a new enum value. **Manual (x, y) entry only in v1** (author green-lit; spreadsheet paste = possible fast-follow). `allowedRegressionModels` shipped as designed (default all, empty = off). Conventions implemented as specified: exponential `y = a·bˣ` with r² on the log-transformed fit, quadratic labeled R², y ≤ 0 gets an inline message. The engine (`regression.ts`) and table (`data-table.ts`) are standalone modules per the pre-emptive-cheap-moves list. Entry bundle cost: +~3 KiB gz (267 total).
6. **Expression list → two-column graphing layout (Stage 4, 2026-07-05; author green-lit over the stacked card).** Graphing mode is list-left / board-right (30rem card); the Data view swaps the left column via CSS. Rows classify as curve / `(a, b)` point / `a = 3` slider (single letter except x/y/e, constant RHS) via `classifyExpression()` in the evaluate seam; sliders live in ONE stable scope object plot closures capture, so drags mutate it and `board.refresh()` re-samples — no rebuild. `maxExpressions` restriction (optional; absent = unlimited). Keypad `=` inserts itself in graphing (no "equals" on a graphing calculator). Known v1 gap: no `,` keypad key — touch-only students can't type a point; polish candidate alongside spreadsheet paste.

7. **Graphing UX overhaul (2026-07-06, author-driven; narrative in HISTORY).** Decisions worth keeping: (a) **the panel floats** as a fixed, header-draggable window on published pages, gated by a `hooks.floating` flag so the **editor preview stays in-flow** — a fixed preview would jump to a viewport corner. (b) **Graphing input is MathLive's virtual keyboard, not a custom pad**: rendered *inside* the panel via `mathVirtualKeyboard.container = panel`, a single `'numeric'` layout (no matrix/greek switcher), the ☰ menu + built-in in-field toggle hidden via CSS `::part(...)` (`menuItems = []` throws on an unmounted field — use CSS). Scientific mode keeps its button pad. (c) **Bare no-variable rows are calculations** (`= value`, no plot); `y=…` still plots. (d) **Sticky axes** via JSXGraph `position: 'sticky'` keep the scale on-screen when an axis pans away — no custom overlay needed. (e) Responsive keyboard via a **CSS size-query container** on the panel (scales MathLive's `--keycap-*` vars; also activates its own `4cqw` font default). (f) Panel geometry (size/position/split) is remembered in a **module-level** var, never localStorage — calculator state persistence stays deferred.
