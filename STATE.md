...
- **Import-format registry seam** (eng-reviewed 2026-07-21, deferred to a follow-on): a declarative fence/blank-grammar registry that the parser vocabulary AND a prompt/doc generator both consume, plus a schema↔registry cross-check test — so `markdownToTiptap.ts` / `markdownImportPrompt.ts` / `docs/markdown-import-format.md` can't drift (today they're hand-synced, guarded only by the three-way `markdownImportPrompt.test.ts`). Full seam + cost estimate in [capability-inventory.md](docs/capability-inventory.md) §5 (~2.5–3 days full; ~1 day for the afternoon cut). Also open from the same inventory: the editor's live `{{…}}` input rule (`Blank.ts`) doesn't interpret the new `?`/`!`/`==` sigils (import-only, same as `~`/`=`) — the registry is where to unify them.
