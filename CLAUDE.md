...
- **[TOMBSTONE, S9 Drops 1–4, 2026-08-14] The renderer world is GONE: `packages/renderer`, its runtime, `bundle:renderer`, the renderer CI drift step, `_shared/renderer.bundle.js`, the graph-kit manifest, and the R2 upload-ordering rule all died with published pages.** The viewer + editor build the graph kit from the workspace — no upload is ever needed for kit changes. Remaining R2 machinery (`upload:graph-kit`, `build:fonts`, `build:mathlive-fonts`, `.env.r2`, the bucket) dies at the D-13 teardown station, gated on the MathLive-font check (OV-11).
