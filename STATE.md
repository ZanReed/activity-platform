...
3. **Calculator UX work changed the kit → re-upload + redeploy (current head `968f629`).** Several post-`EQ5MV42W` passes (board pan/zoom/resize, floating draggable panel, then the graphing overhaul: MathLive keyboard, splitter, hover-trace, calc results) each re-hashed the kit. The **committed manifest is now `graph-kit-KWYAZXJZ.js`** — that is the one to ship. **Order: upload FIRST, then redeploy** (the reverse left a window where the function pointed at a not-yet-uploaded hash and the summon button 404'd). So: `pnpm build:graph-kit` (auto-loads `.env.r2` now — no creds to paste), confirm three `Uploaded:` lines, then `supabase functions deploy publish-activity --project-ref dtqutpdplefmufrrakxs`, then re-publish (action #2). Older hashes stay on R2, so already-published pages keep working until re-published.
...
| Calculator tool (Phase 2.7) | ✅ Stages 1–4 + full graphing UX overhaul (floating panel, in-panel MathLive keyboard, splitter, hover-trace, sticky axes, calc results) — all browser-verified at `/dev/calculator` ([HISTORY 2026-07-06](docs/HISTORY.md)). ⏳ Pending author action #3 (upload kit `KWYAZXJZ` + `publish-activity` redeploy + re-publish) to reach published pages. Next: Stage 5 (graded interactions) or Stage 4 polish (comma key, spreadsheet paste) |
