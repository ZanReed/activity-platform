...
| **2** | Polish the loop | ~~Markdown import~~ (shipped early in Phase 1), ~~image upload~~, ~~reference panel~~, ~~vocabulary definitions~~, ~~multiple choice~~ (shipped 2026-07-10, with numeric blanks), better submission analytics, self-signup, matching + ordering question types |
...
**Common question types beyond fill-in-blank**: multiple choice (single and multi-select) ✅ **SHIPPED 2026-07-10**; matching pairs and ordering / sequencing remain. These are basic question types in nearly every subject — and historically far more common than fill-in-blank in non-STEM classrooms — but they don't fit `BlankResponse` because the answer isn't a string. Each new type follows the parallel-map pattern established by graphs: a new `choices` / `matches` / `orderings` map on `SubmissionResponses`, a new block type in the discriminated union, and a new NodeView. **As-built correction from the MC ship:** the wire bump landed at v5 (graphs took v3/v4 first), and a selection-shaped question gets its own runtime machinery (`runtime/mcs.ts`, scored all-or-nothing by set equality), NOT a strategy in the `evaluateAnswer` dispatch — that dispatch is for typed-blank answer formats (list/numeric). Expect matching/ordering to follow the mcs.ts shape. Architecturally simpler than graphs since the response is a selection / arrangement, not geometric coordinates.
