...
export function renderNumberLineSvg(
  ...
  return (
    ...
    // See the note in graph-svg.ts: the count is what lets the print-parity
    // gate distinguish an empty line a student marks from a display figure
    // that already carries its marks, on either surface.
    ` data-drawables="${marks.length}"` +
