...
**📌 THE ONE OPEN ACTION: observe the cron's first fire.** Both jobs were registered 2026-08-05 (`purge-soft-deleted` `0 3 * * *`, `analytics-maintenance` `30 3 * * *`; both `active`, `database=postgres username=postgres`, `cron.log_run=on`) but **had not yet run** when last checked. Check the morning after — the prediction below is deliberately falsifiable, so if it disagrees, something is wrong.

- 03:00 `purge_soft_deleted` — its first execution ever. Expect it to COMPLETE without abort (exactly what 0023 fixed) and purge nothing: nothing is soft-deleted past its window.
- 03:30 `run_analytics_maintenance` — expect a ledger row with **`stale_cache_rows_deleted = 3`**, `section_check_rows = 0`, `census_versions = 0`. The GC heuristic is verified safe: the newest cache row carries the genuinely current rev `1-f8328527`, so the sweep targets the three older `1-5dbcb651` orphans and keeps the two good rows — not the inverse.
...
**Otherwise EMPTY.** Verified against the live project rather than assumed (2026-08-04/05): migrations applied **through 0026**; `ingest-submission` v42 / `get-feedback` v4 / `get-activity` v8 all `verify_jwt:false`; `check-activity` v5 + `publish-activity` v122 both `verify_jwt:true`; kit manifest `graph-kit-R5LUPQJS.js` committed. **S7 is COMPLETE** — migration applied, function deployed, backfill run (24 versions censused), live e2e probe 8/0. **S8 needs no author action at all** (repo + CI only). Verification narratives for S7, 0021–0026 and the retention arc are archived in [HISTORY.md](docs/HISTORY.md).
