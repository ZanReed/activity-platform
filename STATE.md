...
0. **⏭ NOW: push the 4 unpushed commits** (`9b78496` recalibration · `d9a93f6` fonts fix · the retro re-run + STATE commits) → CI gate. The fonts fix is the one with product effect: **production math fields were 404ing their glyph fonts** (detail below). Tab-lock family: **four consecutive 0-flaky runs** since `settledLockHolders()` — treat as closed unless a fifth sighting appears.
...
**Cutover gates C1–C15 — 14 of 15 CLOSED.** Still open: **gate 4** (seed `student_domain` + live student-branch verify — pending action 2). Gate 9's mechanical halves closed 2026-08-14 (recalibration + retro re-run); only its 150 KiB **ruling** remains, and a ruling gates nothing mechanical. Gate 13's cutover half is satisfied; its counsel read (D24) gates classrooms, not cutover. Rulings + detail in [findings-backlog.md](docs/reviews/findings-backlog.md) → RULINGS; plan in [s9-cutover.md](docs/design/s9-cutover.md) (D-1…D-14 + eng §7 + DX §8/§9 + design §10).
