...
**A1b — Stale-result detection (new, from outside voice).** Because math blanks are graded through the held reference and a stored result is load-bearing across reloads, `BlankState` must record **the value that was graded** (not just the boolean), so a restored blank whose typed value no longer matches the graded value is re-graded rather than trusted. This widens `BlankState` → **bump `STORAGE_SCHEMA_VERSION`** (load-null-on-mismatch = fresh state, correct).
...
- **Wire / `ingest-submission` redeploy** — none (Model B reuses the `blanks` map; wire stays v9). (`STORAGE_SCHEMA_VERSION` DOES bump — A1b adds a graded-value field to `BlankState`.)
...
- [ ] **T4 (P1)** — runtime/storage — A1b: add graded-value to `BlankState`; **bump `STORAGE_SCHEMA_VERSION`**; re-grade on restore when typed≠graded value. Files: `packages/renderer/src/runtime/storage.ts`. Verify: jsdom CRITICAL (stale-restore).
...
- [ ] **T7 (P2)** — editor — blank discoverability signifier. **Form RESOLVED via `/plan-design-review` 2026-07-19: HYBRID, kept lean.** (1) Inline teacher = extend the fill-in-blank `PromptField`: empty body reads "Type the sentence…  ( `__` makes a blank )"; once there's text but **zero blanks**, show trailing faint "type `__` to make a blank"; fades permanently once the block has ≥1 blank (show-when-no-blank, hide-forever-after). Pure `.prompt-field`/`--ed-faint` reuse, no new component. (2) Mouse/keyboard entry = ONE "Insert blank" action on the existing `BlockQuickBarHost` (drops a blank at caret) — no new in-text widget, no `+` in the sentence (which would collide with the gutter insert-`+` and mimic a real blank's underline). Rejected: standalone `+`-underline in-text widget (off-idiom, new a11y surface). Verify: e2e ★★ (hint appears with text+no-blank, gone after a blank; quick-bar Insert-blank inserts at caret).
- [ ] **T8 (P1)** — deploy prep — `pnpm upload:graph-kit` (mathEquivalent lands in the kit) BEFORE `publish-activity` redeploy; `pnpm bundle:renderer` committed in-commit (runtime strategy + storage bump). No ingest redeploy (wire v9). Author-run.
