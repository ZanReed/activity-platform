...
**⚠ APPLY migration `0025_student_dormancy.sql`** (`supabase db push`) — starts the account clock and closes the retention arc. Students become purge-eligible after 400 days with no active class membership, **derived live** (no column, no sweep, nothing to drift); `users.deleted_at` is never written by the job because `join_class` refuses accounts that have it set. No schema change. Proven by fixture in a rolled-back transaction — five real student accounts through the real signup trigger, six-case matrix all correct, nothing committed. No redeploy (pg_cron calls it in-DB). After applying, run `scripts/verify-0025.sql` — **section 0 first**; section C reports a PASS as a `P0001` error.
